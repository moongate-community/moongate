...
public sealed class Files
{
    ...
    public static void LoadMulPath()
    {
        ...
        Dictionary<string, string> onDisk = BuildCaseInsensitiveFileMap(RootDir);

        foreach (string file in _uoFiles)
        {
            MulPath[file] = onDisk.TryGetValue(file, out string actual) ? actual : string.Empty;
        }
    }
    ...
    public static void SetMulPath(string path)
    {
        ...
        foreach (string file in _uoFiles)
        {
            ...
            if (!string.IsNullOrEmpty(MulPath[file]))
            {
                ...
                {
                    if (onDisk.TryGetValue(MulPath[file], out string mapped))
                    {
            ...
            MulPath[file] = onDisk.TryGetValue(file, out string actual) ? Path.Combine(RootDir, actual) : string.Empty;
        }
    }
