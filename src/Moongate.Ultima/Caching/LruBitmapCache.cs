...
public sealed class LruBitmapCache : IDisposable
{
    ...
    private readonly LinkedList<KeyValuePair<int, UltimaBitmap>> _list =
        new LinkedList<KeyValuePair<int, UltimaBitmap>>();
    private readonly Dictionary<int, LinkedListNode<KeyValuePair<int, UltimaBitmap>>> _map;
    ...
    public LruBitmapCache(int capacity)
    {
        ...
        _capacity = capacity;
        _map = new Dictionary<int, LinkedListNode<KeyValuePair<int, UltimaBitmap>>>(Math.Min(capacity, 4096));
    }
    ...
    public bool TryGet(int key, out UltimaBitmap value)
    {
        lock (_lock)
        {
    ...
    public void Set(int key, UltimaBitmap value)
    {
        ...
        lock (_lock)
        {
            ...
            {
                UltimaBitmap previous = existing.Value.Value;
                _list.Remove(existing);
                var replacement = new LinkedListNode<KeyValuePair<int, UltimaBitmap>>(new KeyValuePair<int, UltimaBitmap>(key, value));
            ...
            var node = new LinkedListNode<KeyValuePair<int, UltimaBitmap>>(new KeyValuePair<int, UltimaBitmap>(key, value));
            _list.AddFirst(node);
            _map[key] = node;
    ...
    public bool Remove(int key)
    {
        lock (_lock)
        {
            if (_map.TryGetValue(key, out var node))
            {
                UltimaBitmap bmp = node.Value.Value;
                _list.Remove(node);
                _map.Remove(key);
                if (DisposeOnEvict)
                {
