...
public sealed class LruBitmapCache : IDisposable
{
    ...
    private readonly LinkedList<KeyValuePair<int, UltimaBitmap>> _list = new();
    ...
    public void Set(int key, UltimaBitmap value)
    {
        ...
        lock (_lock)
        {
            ...
            if (_map.TryGetValue(key, out var existing))
            {
                ...
                var replacement = new LinkedListNode<KeyValuePair<int, UltimaBitmap>>(new(key, value));
            ...
            var node = new LinkedListNode<KeyValuePair<int, UltimaBitmap>>(new(key, value));
            _list.AddFirst(node);
            _map[key] = node;
