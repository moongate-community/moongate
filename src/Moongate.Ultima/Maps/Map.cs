...
public sealed class Map
{
    ...
    /// <param name="statics">Include statics in rendering</param>
    /// <param name="altitudeMode">Altitude rendering mode</param>
    public unsafe void GetImageWithAltitude(int x, int y, int width, int height, UltimaBitmap bmp, bool statics, MapAltitudeMode altitudeMode)
    {
        int stride = bmp.Stride;
        int blockStride = stride << 3;
            ...
            for (int oy = 0, by = y; oy < height; ++oy, ++by, pStart += blockStride)
            {
                ...
                for (int ox = 0, bx = x; ox < width; ++ox, ++bx)
                {
                    ...
                    fixed (sbyte* pAlt = altitudeData)
                    {
                        ...
                        for (int col = 0; col < 8; ++col) *pRow0++ = AltitudeToGray(*pa++);
                        for (int col = 0; col < 8; ++col) *pRow1++ = AltitudeToGray(*pa++);
                        for (int col = 0; col < 8; ++col) *pRow2++ = AltitudeToGray(*pa++);
                        for (int col = 0; col < 8; ++col) *pRow3++ = AltitudeToGray(*pa++);
                        for (int col = 0; col < 8; ++col) *pRow4++ = AltitudeToGray(*pa++);
                        for (int col = 0; col < 8; ++col) *pRow5++ = AltitudeToGray(*pa++);
                        for (int col = 0; col < 8; ++col) *pRow6++ = AltitudeToGray(*pa++);
                        for (int col = 0; col < 8; ++col) *pRow7++ = AltitudeToGray(*pa++);
                    }
                }
            }
        }
