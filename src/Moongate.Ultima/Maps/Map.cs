...
public sealed class Map
{
    ...
    {
        var indexPath = Files.GetFilePath($"staidx{map.FileIndex}.mul");
        ...
        var staticsPath = Files.GetFilePath($"statics{map.FileIndex}.mul");
        ...
        if (staticsPath != null)
        {
            staticsStream = new(staticsPath, FileMode.Open, FileAccess.Read, FileShare.Read);
            staticsReader = new(staticsStream);
        }
        else
        {
            return;
    ...
    {
        var stride = bmp.Stride;
        var blockStride = stride << 2; // 4 rows per block
             ...
             ++oy, ++by, pStart += blockStride)
        {
            ...
            for (int ox = 0,
                 ...
                 ++ox, ++bx)
            {
                var data = GetRenderedBlockHalf(bx, by, statics);
    ...
    {
        var stride = bmp.Stride;
        var blockStride = stride << 1; // 2 rows per block
             ...
             ++oy, ++by, pStart += blockStride)
        {
            ...
            for (int ox = 0,
                 ...
                 ++ox, ++bx)
            {
                var data = GetRenderedBlockQuarter(bx, by, statics);
    ...
    public void PreloadRenderedBlock(int x, int y, bool statics)
    {
        ...
        var cache = EnsureColorCacheArray(statics);
    ...
    public static void Reload()
    {
        ...
        foreach (var m in new[] { Felucca, Trammel, Ilshenar, Malas, Tokuno, TerMur })
        {
            m.ClearMipCaches();
    ...
    public void ReportInvalidMapIDs(string reportFile)
    {
               ...
               ))
        {
            ...
            for (var x = 0; x < Width; ++x)
            {
                ...
                {
                    var currentTile = Tiles.GetLandTile(x, y);
                    ...
                    foreach (var currentStatics in Tiles.GetStaticTiles(x, y))
                    {
               ...
               ))
        {
            ...
            for (var x = 0; x < Width; ++x)
            {
                for (var y = 0; y < Height; ++y)
                {
                    ...
                    foreach (var currentStatic in Tiles.GetStaticTiles(x, y))
                    {
    ...
    public static void RewriteMap(string path, int mapIndex, int width, int height)
    {
        ...
        var mulPath = Path.Combine(path, $"map{mapIndex}.mul");
    ...
    private sbyte[] GetAltitudeBlockCached(int x, int y, bool statics)
    {
        ...
        if (cache == null)
        {
            ...
            if (UseDiff)
            {
                if (statics)
                {
                    _altitudeCache = cache;
                }
                else
                {
                    _altitudeCacheNoStatics = cache;
                }
            }
            else
            {
                if (statics)
                {
                    _altitudeCacheNoPatch = cache;
                }
                else
                {
                    _altitudeCacheNoStaticsNoPatch = cache;
                }
            }
        }
    ...
    private ushort[] GetLitBlock(int x, int y, bool statics)
    {
        ...
        if (cache == null)
        {
            ...
            else
            {
                if (statics)
                {
                    _litCacheNoPatch = cache;
                }
                else
                {
                    _litCacheNoStaticsNoPatch = cache;
                }
            }
        }
        ...
        if (data == null)
        {
            var colors = GetRenderedBlock(x, y, statics);
            var altitudes = GetAltitudeBlockCached(x, y, statics);
    ...
    private ushort[] GetRenderedBlockHalf(int x, int y, bool statics)
    {
        ...
        if (cache == null)
        {
            ...
            else
            {
                if (statics)
                {
                    _cacheHalfNoPatch = cache;
                }
                else
                {
                    _cacheHalfNoStaticsNoPatch = cache;
                }
            }
        }
        ...
        if (data == null)
        {
            var full = GetRenderedBlock(x, y, statics);
    ...
    private ushort[] GetRenderedBlockQuarter(int x, int y, bool statics)
    {
        ...
        if (cache == null)
        {
            ...
            else
            {
                if (statics)
                {
                    _cacheQuarterNoPatch = cache;
                }
                else
                {
                    _cacheQuarterNoStaticsNoPatch = cache;
                }
            }
        }
...
#region Altitude Map Rendering
        ...
        if (altitudeMode == MapAltitudeModeType.Altitude)
        {
                 ...
                 ++oy, ++by, pStart += blockStride)
            {
                ...
                for (int ox = 0,
                     ...
                     ++ox, ++bx)
                {
                    var altitudeData = GetAltitudeBlockCached(bx, by, statics);
                    ...
                    fixed (sbyte* pAlt = altitudeData)
                    {
                        ...
                        {
                            *pRow0++ = AltitudeToGray(*pa++);
                        }

                        for (var col = 0; col < 8; ++col)
                        {
                            *pRow1++ = AltitudeToGray(*pa++);
                        }

                        for (var col = 0; col < 8; ++col)
                        {
                            *pRow2++ = AltitudeToGray(*pa++);
                        }

                        for (var col = 0; col < 8; ++col)
                        {
                            *pRow3++ = AltitudeToGray(*pa++);
                        }

                        for (var col = 0; col < 8; ++col)
                        {
                            *pRow4++ = AltitudeToGray(*pa++);
                        }

                        for (var col = 0; col < 8; ++col)
                        {
                            *pRow5++ = AltitudeToGray(*pa++);
                        }

                        for (var col = 0; col < 8; ++col)
                        {
                            *pRow6++ = AltitudeToGray(*pa++);
                        }

                        for (var col = 0; col < 8; ++col)
                        {
                            *pRow7++ = AltitudeToGray(*pa++);
                        }
                    }
                }
            }
    ...
    {
        var gray = (byte)(altitude + 128) >> 3;

        return (ushort)(0x8000 | (gray << 10) | (gray << 5) | gray);
    }
    ...
    private sbyte[] GetAltitudeBlock(int x, int y, bool drawStatics)
    {
        ...
        var tiles = _tiles.GetLandBlock(x, y, UseDiff);
                ...
                fixed (Tile* ptTiles = tiles)
                {
                    ...
                    for (var k = 0; k < 8; ++k)
                    {
                        for (var p = 0; p < 8; ++p)
                        {
                            ...
                            if (drawStatics)
                            {
                                var statics = _tiles.GetStaticBlock(x, y, UseDiff);
                                var curStatics = statics[p][k];
    ...
    private static ushort[] ProcessBlockWithAltitude(ushort[] colors, sbyte[] altitudes)
    {
        ...
        for (var y = 0; y < 8; y++)
        {
            for (var x = 0; x < 8; x++)
            {
                ...
                var dx = GetAltitudeDifference(altitudes, x, y, 1, 0, settings.GradientSmoothing);
                var dy = GetAltitudeDifference(altitudes, x, y, 0, 1, settings.GradientSmoothing);

                // Calculate surface normal with configurable nz
                var nx = -dx;
                var ny = -dy;
                var nz = settings.NormalZ + (maxSlope - 10) * 0.5f; // Adjust based on intensity
                ...
                var dotProduct = nx * lightX + ny * lightY + nz * lightZ;
                dotProduct = Math.Clamp(dotProduct, 0.0f, 1.0f);

                // Apply configurable brightness range
                var baseIntensity = 20.0f / maxSlope; // Higher maxSlope = less intense
                var range = settings.BrightnessRange * baseIntensity;
                var multiplier = 1.0f + (dotProduct - 0.5f) * 2.0f * range;

                // Clamp to safe range based on brightness range
                var minMult = 1.0f - range;
                var maxMult = 1.0f + range;
    ...
    {
        var x1 = Math.Max(0, Math.Min(7, x - dx));
        var y1 = Math.Max(0, Math.Min(7, y - dy));
        var x2 = Math.Max(0, Math.Min(7, x + dx));
        var y2 = Math.Max(0, Math.Min(7, y + dy));
