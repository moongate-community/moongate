...
public static class Animations
{
    ...
    private static FileIndex _fileIndex4 = new("Anim4.idx", "Anim4.mul", 0x20000, -1);
    private static FileIndex _fileIndex5 = new("Anim5.idx", "Anim5.mul", 0x20000, -1);
    private static FileIndex _fileIndex6 = new("Anim6.idx", "Anim6.mul", 0x20000, -1);
    ...
    )
    {
        ...
        var lookupHue = hue;
        var cacheKey = BuildAnimationKey(body, action, direction, fileType, firstFrame, lookupHue, false);

        if (Cache.TryGet(cacheKey, out var cachedFrames))
        {
    ...
    public static MobType GetBodyMobType(int body, int fileType)
    {
        if (MobTypes.IsLoaded)
        {
            ...
            var serverBody = fileType == 1 ? body : BodyConverter.GetTrueBody(fileType, body);

            if (serverBody >= 0 && MobTypes.TryGet(serverBody, out var mt, out _))
            {
                return mt;
            }
        }
    ...
    public static void Reload()
    {
        ...
        _fileIndex = new("Anim.idx", "Anim.mul", 0x40000, 6);
        _fileIndex2 = new("Anim2.idx", "Anim2.mul", 0x10000, -1);
        _fileIndex3 = new("Anim3.idx", "Anim3.mul", 0x20000, -1);
        _fileIndex4 = new("Anim4.idx", "Anim4.mul", 0x20000, -1);
        _fileIndex5 = new("Anim5.idx", "Anim5.mul", 0x20000, -1);
        _fileIndex6 = new("Anim6.idx", "Anim6.mul", 0x20000, -1);
