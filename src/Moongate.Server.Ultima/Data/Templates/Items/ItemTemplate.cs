...
/// <summary>An authored item definition, one TOML entry under <c>templates/items/</c>.</summary>
public class ItemTemplate
{
    /// <summary>The stable id a loot table, a spawn or the <c>additem</c> command names this template by.</summary>
    ...
    /// stackability, are read from it through <c>IItemCatalog</c> at the point of use, not restated here.
    /// </summary>
    public Serial ItemId { get; set; }

    /// <summary>Overrides tiledata's own name, which is often generic ("a book"), when set.</summary>
    public string? Name { get; set; }

    /// <summary>A designer's note. Read by nobody at runtime.</summary>
    public string? Comment { get; set; }

    /// <summary>A fixed rarity, or a policy for picking one at random on every spawn.</summary>
    ...
    /// well-known functions exists, <c>on_use</c>, <c>on_equip</c>, <c>on_unequip</c> and so on; a
    ...
    /// <summary>Maximum item count for a container template; null for anything that is not a container.</summary>
    public int? MaxItems { get; set; }

    /// <summary>Maximum carried weight for a container template; null for anything that is not a container.</summary>
