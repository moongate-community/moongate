...
public sealed class AccountAdminAccessService : IAccountAdminAccessService
{
    ...
    public AccountAdminAccessService(MoongatePersistenceService persistence, IDataAccess<AccountEntity> accounts,
        IAdminSessionStore sessions, AdminSessionOptions options)
    {
    ...
    public async Task<AdminLoginResult?> LoginAsync(string username, string password, CancellationToken token = default)
    {
        ...
        try
        {
            await _persistence.ExecuteInTransactionAsync(PersistenceDatabaseTarget.Accounts, async tx =>
            {
                ...
                if (account is null || account.IsLocked || !account.CanAccessApi || !Enum.IsDefined(account.AccountType) ||
                    !StringComparer.Ordinal.Equals(account.Username, username) || !HashUtils.VerifyPassword(password, account.HashPassword))
                {
                    return;
                }
                ...
                var session = await _sessions.IssueAsync(new(account.Id, account.Username, account.AccountType),
                    gate.Generation, digest, _options.Lifetime, token);
                account.LastLoginAt = DateTime.UtcNow;
                await tx.GetDataAccess<AccountEntity>().UpsertAsync(account, token);
                result = new(rawToken, session.ExpiresAt, new(account.Id, account.Username, account.AccountType,
                    account.CanAccessApi, account.IsLocked, account.CreatedAt));
            }, token);
            return result;
        }
        catch
        {
            if (digest is not null)
            {
                ...
                catch (Exception)
                {
                    ...
                    _logger.Warning("Could not remove an undisclosed administrative session after a failed login transaction");
                }
            }
            throw;
        }
    ...
    public Task UpdateAccessAsync(Serial accountId, AccountAccessOptions options, CancellationToken token = default)
    {
        ...
        return MutateAsync(accountId, account =>
        {
    ...
    public Task RevokeSessionsAsync(Serial accountId, CancellationToken token = default)
        => _persistence.ExecuteInTransactionAsync(PersistenceDatabaseTarget.Accounts, async tx =>
        {
