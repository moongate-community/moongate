...
/// each priority level.
/// </summary>
public class EventLoopService : IEventLoopService, IMetricsProvider
{
    ...
    /// </summary>
    public EventLoopService(IEventBusService eventBusService, EventLoopConfig config)
    {
    ...
    public async Task StopAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.Information("Stopping event loop service...");
            ...
            try
            {
                ...
                if (_loopTask != null)
                {
                    ...
                    {
                        _logger.Warning("Event loop task did not complete within timeout period");
                    }
                    else
                    {
                        _logger.Information("Event loop task completed successfully");
                    }
                }
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Error while stopping event loop service");
            }
            finally
            {
    ...
    private void ProcessTick()
    {
        ...
        {
            _logger.Error(ex, "Error in event loop tick");
        }
        finally
        {
