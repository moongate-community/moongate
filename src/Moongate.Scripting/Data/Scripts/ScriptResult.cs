...
{
    /// <summary>Gets the shared result of a call that suspended on <c>wait</c>: the coroutine is parked on the timer wheel and will be resumed there, so it carries no values and no error.</summary>
    ...
    /// <summary>Gets the values the function returned, converted to CLR types. Empty unless <see cref="Kind"/> is <see cref="ScriptResultKind.Completed"/>.</summary>
    ...
    /// <summary>Creates the result of a function that failed. The failure has already been logged and published when the engine returns one of these.</summary>
