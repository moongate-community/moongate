...
Memory is only partly bounded. `string.rep` refuses a result longer than `MaxStringLength` (16,777,216 characters by default, `max_string_length` in the server's `[scripting]` section; Lua strings here are UTF-16, so that is 32 MiB of text) with a script error, counted as a memory cap hit in the metrics. Everything else allocates freely under the instruction budget: a table constructor, or a loop that doubles a string with `..`, can build far more than the budget suggests before it is stopped, and there is no cap on the total memory a state may hold.
