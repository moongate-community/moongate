...
/// <summary>Container registrations that tell the script engine what to publish to Lua at startup. Call them before the engine starts; the registry is read once, while it binds.</summary>
public static class ContainerScriptingExtensions
{
    extension(Container container)
    {
        ...
        /// <typeparam name="TEnum">The enum to publish; its members become the table's keys.</typeparam>
        /// <returns>The same container, for chaining.</returns>
