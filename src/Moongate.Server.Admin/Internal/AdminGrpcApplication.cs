...
internal static class AdminGrpcApplication
{
    ...
    public static void AddServices(IServiceCollection services, AdminApiConfig config, IAdminSessionStore sessions,
        IAdminLoginThrottle throttle, IAdminServerInfoProvider info, IAccountService? accounts, IAccountAdminAccessService? authority)
    {
        ...
        services.AddAuthentication(AdminAuthorizationPolicies.Scheme)
            .AddScheme<AuthenticationSchemeOptions, AdminTokenAuthenticationHandler>(AdminAuthorizationPolicies.Scheme, _ => { });
        services.AddAuthorization(options => options.AddPolicy(AdminAuthorizationPolicies.AccountAdministration,
    ...
    public static void Configure(WebApplication app, ServerMode mode, AdminRequestGate gate)
    {
        ...
        if ((mode & ServerMode.Login) != 0)
        {
            ...
            app.MapGrpcService<AdminAccountsGrpcService>().RequireAuthorization(AdminAuthorizationPolicies.AccountAdministration);
            app.MapGrpcService<AdminAccountSessionsGrpcService>().RequireAuthorization(AdminAuthorizationPolicies.AccountAdministration);
        }
    }
}
