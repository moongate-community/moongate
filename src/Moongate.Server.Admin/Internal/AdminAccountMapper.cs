...
internal static class AdminAccountMapper
{
    ...
    {
        if (string.IsNullOrWhiteSpace(username) || username.Length > 255 || username.Contains('\0') ||
            string.IsNullOrWhiteSpace(password) || password.Contains('\0') || Encoding.UTF8.GetByteCount(password) > 1024)
        {
    ...
    public static AccountCreateOptions ToCreateOptions(CreateAccountRequest request)
    {
        ...
        return new()
        {
            ...
            AccountType = !request.HasAccountType ? DomainAccountType.Regular : request.AccountType switch
            {
    ...
    public static AccountSummary ToCreateResponse(AccountCreateResult result)
    {
        ...
        throw new RpcException(result.ResultType == AccountCreateResultType.UsernameAlreadyExists
    ...
    public static AccountSummary ToSummary(AccountEntity account)
        => ToSummary(new AdminAccountSnapshot(account.Id, account.Username, account.AccountType,
