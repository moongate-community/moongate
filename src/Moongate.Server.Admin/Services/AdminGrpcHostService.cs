...
public sealed class AdminGrpcHostService : IAdminApiService, IAsyncDisposable
{
    ...
    private readonly ILogger _logger = Log.ForContext<AdminGrpcHostService>();
    ...
    public async Task StartAsync()
    {
        ...
        try
        {
            ...
            _logger.Information(
                "Administration gRPC listening on {Address}:{Port} for {Mode}; awaiting server readiness",
        ...
        catch (Exception)
        {
            ...
            throw new InvalidOperationException(
                "Cannot start admin_api listener. Check bind address, port and service registration."
            );
        }
    }
    ...
    private X509Certificate2 LoadCertificate()
    {
        ...
        {
            throw new InvalidOperationException(
                "Invalid admin_api certificate. Configure a readable, unexpired server PFX with a private key and matching password."
            );
        }
    }
