    ...
    public static bool TryRead(string sql, out List<SchemaSqlStatement> statements)
    {
        ...
        for (var i = 0; i < sql.Length;)
        {
            ...
            if (sql.AsSpan(i).StartsWith("/*"))
            {
                ...
                {
                    if (sql.AsSpan(i).StartsWith("/*")) { depth++; i += 2; }
                    else if (sql.AsSpan(i).StartsWith("*/")) { depth--; i += 2; }
                    else { i++; }
                }
            ...
            if (sql[i] == ';')
            {
                if (tokens.Count > 0) { statements.Add(new(sql[start..(i + 1)], tokens.ToArray())); }
                tokens.Clear();
                start = ++i;
                continue;
            }
            ...
            {
                var begin = i++;
                while (i < sql.Length && (char.IsAsciiLetterOrDigit(sql[i]) || sql[i] == '_')) { i++; }
                tokens.Add(sql[begin..i].ToUpperInvariant());
                continue;
            }
            tokens.Add(sql[i++].ToString());
        }
        if (tokens.Count > 0) { statements.Add(new(sql[start..], tokens.ToArray())); }
        return true;
    }
}
