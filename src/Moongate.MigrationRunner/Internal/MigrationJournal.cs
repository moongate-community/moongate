...
internal sealed class MigrationJournal : IJournal
{
    ...
    {
        return _connectionManager().ExecuteCommandsWithManagedConnection(factory =>
        {
            using var command = factory();
            command.CommandText = "SET LOCAL standard_conforming_strings = on; SELECT pg_advisory_xact_lock(1296516941)";
            command.ExecuteNonQuery();
            var applied = MigrationHistory.ReadAsync(() => (DbCommand)factory(), _catalog.Target).GetAwaiter().GetResult();
    ...
    public void StoreExecutedScript(SqlScript script, Func<IDbCommand> dbCommandFactory)
    {
        ...
        using var command = dbCommandFactory();
        command.CommandText = "INSERT INTO moongate_migrations.history(target, component, script, checksum) VALUES (@target, @component, @script, @checksum)";
