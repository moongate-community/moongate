...
internal static class PacketMetadata
{
    ...
    {
        if (!packetType.IsClass || packetType.IsAbstract || packetType.ContainsGenericParameters || !typeof(IPacket).IsAssignableFrom(packetType))
        {
        ...
        var attribute = packetType.GetCustomAttributes(typeof(PacketHandlerAttribute), false)
            ...
            .SingleOrDefault() ?? throw Error(packetType, "is missing PacketHandlerAttribute");
        var incomingContract = packetType.GetInterfaces().Any(
            candidate => candidate.IsGenericType
                         && candidate.GetGenericTypeDefinition() == typeof(IIncomingPacket<>)
        ...
        return attribute.Sizing switch
        {
            PacketSizing.Fixed when attribute.Length is >= 1 and <= ushort.MaxValue
                => new PacketDescriptor(attribute.OpCode, attribute.Sizing, attribute.Length, attribute.Length, direction, packetType, attribute.Description),
            ...
            PacketSizing.Variable when attribute.Length == -1 && attribute.MinimumLength is >= 3 and <= ushort.MaxValue
                => new PacketDescriptor(attribute.OpCode, attribute.Sizing, null, attribute.MinimumLength, direction, packetType, attribute.Description),
            PacketSizing.Variable => throw Error(packetType, "has invalid variable sizing; Length must be -1 and MinimumLength must be 3..65535"),
