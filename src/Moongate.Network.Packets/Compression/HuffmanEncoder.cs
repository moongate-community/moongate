namespace Moongate.Network.Packets.Compression;

/// <summary>
///     Huffman compression of the data the game server sends, as the UO client expects it once the game login is
///     accepted. The code table is fixed by the protocol and identical in every UO server: 256 byte codes plus a
///     terminal code. Only the client decompresses, so there is no decoder here.
/// </summary>
public static class HuffmanEncoder
{
    /// <summary>
    ///     UO packets may not exceed 64 KB.
    /// </summary>
    ...
    /// <summary>
    ///     Fixed overhead, in bits, of the terminal code written once per call.
    /// </summary>
    private const int TerminalCodeLength = 4;

    /// <summary>
    ///     Above this input length the output cannot fit in <see cref="BufferSize" />.
    /// </summary>
    private const int DefiniteOverflow = (BufferSize * 8 - TerminalCodeLength) / MinimalCodeLength;

    /// <summary>
    ///     (bit length, bit value) pairs indexed by byte value; entry 256 (index 0x200) is the terminal code.
    ///     Best ratio 2/8, worst 11/8.
    /// </summary>
    private static readonly int[] HuffmanTable =
    [
    ...
    /// <summary>
    ///     Gets a conservative upper bound for the compressed size of <paramref name="inputLength" /> bytes.
    /// </summary>
    /// <returns>
    ///     The bound in bytes, or 0 when the input cannot possibly fit in <see cref="BufferSize" />.
    /// </returns>
    public static int CalculateMaxCompressedSize(int inputLength)
    {
        if (inputLength <= 0 || inputLength > DefiniteOverflow)
        {
            return 0;
        }
    ...
    /// </summary>
    /// <param name="input">
    ///     The raw bytes, usually one complete packet.
    /// </param>
    /// <param name="output">
    ///     The destination, sized with <see cref="CalculateMaxCompressedSize" />.
    /// </param>
    /// <returns>
    ///     The number of bytes written, or 0 when the input is too large or the output is too small.
    /// </returns>
    public static int Compress(ReadOnlySpan<byte> input, Span<byte> output)
    {
        ...
        {
            var tableIndex = input[inputIndex++] << 1;
            bitCount += HuffmanTable[tableIndex];
            bitValue = (bitValue << HuffmanTable[tableIndex]) | HuffmanTable[tableIndex + 1];

            while (bitCount >= 8)
            {
                bitCount -= 8;

                if (output.Length < outputIndex + 1)
                {
                    return 0;
                }

                output[outputIndex++] = (byte)(bitValue >> bitCount);
            }
        }

        // Terminal code (entry 256).
        bitCount += HuffmanTable[0x200];
        bitValue = (bitValue << HuffmanTable[0x200]) | HuffmanTable[0x201];
