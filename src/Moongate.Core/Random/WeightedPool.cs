...
public class WeightedPool<T> : IWeightedPool<T>
{
    ...
    private List<WeightedItem<T>> _pool = new();
    ...
    /// </summary>
    /// <param name="random">
    /// A class implementing IRandom that will be used to generate pseudo-random numbers necessary to pick
    /// items from the WeightedPool
    /// </param>
    ...
    public void Add(T item, int weight)
    {
        ...
        var weightedItem = new WeightedItem<T>(item, weight);
    ...
    /// Thrown when the random lookup is greater than the total weight. Could only
    /// happen if a bad implementation of IRandom were provided
    /// </exception>
    public T Choose()
    {
        if (_cloneFunc == null)
        ...
        var item = ChooseRandomWeightedItem();
    ...
    {
        var lookupWeight = _random.Next(1, _totalWeight);
        var currentWeight = 0;
        WeightedItem<T> item = null;

        foreach (var weightedItem in _pool)
        {
