...
public sealed class ApiClient : IApiClient
{
    ...
    public ApiClient(ApiRegistry registry, ApiOptions options, ApiTlsOptions tls, TimeProvider timeProvider)
    {
        ...
        _clock = timeProvider;
        _tls = new ApiTlsPolicy(tls);
        _slots = new SemaphoreSlim(options.MaxConcurrentHandlers, options.MaxConcurrentHandlers);
    }

    public async Task<IApiConnection> ConnectAsync(IPEndPoint endpoint, string targetHost, string expectedPeerId, CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            ...
            if (_admitted >= _options.MaxConnections || _setups.Count >= _options.MaxConcurrentHandshakes) { throw new ApiBusyException(); }
            _admitted++;
            _setups.Add(setup);
        }
        ...
        try
        {
            ...
            var pipeline = new ApiConnectionSetup(_tls, new ApiFrameFramer(_options.MaxFrameLength), (transport, peer) =>
            {
                lock (_gate)
                {
            ...
            }, targetHost, expectedPeerId).Pipeline;
            var transport = await MoongateTcpClient.ConnectConfiguredAsync(endpoint, new TcpClientOptions
            {
                Pipeline = pipeline,
            ...
            Logger.Information("API connected to {Endpoint} as peer {PeerId}: {ContractCount} contracts, {HandlerCount} handlers",
                endpoint, connection.Peer.PeerId, _registry.ContractCount, _registry.HandlerCount);
            return connection;
        }
        finally
        {
    ...
    private async Task RemoveCompletedAsync(ApiConnection connection)
    {
        ...
        lock (_gate) { if (_connections.Remove(connection)) { _admitted--; } }
    }
    ...
    private async Task DisposeCoreAsync()
    {
        ...
        finally { _tls.Dispose(); _slots.Dispose(); _shutdown.Dispose(); }
    }
    ...
    public ValueTask DisposeAsync()
    {
        ...
        lock (_gate)
        {
            ...
            if (_disposeCompletion is null)
            {
                ...
                _disposeResult = ApiShutdown.WaitAsync(_disposeCompletion, ForceClose, _options.ShutdownTimeout, _clock, Remaining);
