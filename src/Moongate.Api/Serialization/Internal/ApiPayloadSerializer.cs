...
internal static class ApiPayloadSerializer
{
    ...
    public static T Deserialize<T>(ReadOnlyMemory<byte> payload)
    {
        ...
        if (result is null || !reader.End) { throw new MessagePackSerializationException("Invalid typed payload."); }
    ...
    public static byte[] Serialize<T>(T value, int maxLength)
    {
        ...
        try { MessagePackSerializer.Serialize(buffer, value, Options); }
    ...
    {
        if (depth > MaxDepth) { throw new ApiProtocolException("Payload nesting limit exceeded."); }
        ...
        switch (reader.NextMessagePackType)
        {
            case MessagePackType.Array:
                ...
                if (count > length - reader.Consumed) { throw new ApiProtocolException("Invalid payload array length."); }

                for (var i = 0; i < count; i++) { Scan(ref reader, length, depth + 1); }
            ...
            case MessagePackType.String:
                ...
                if (text is { } bytes) { StrictUtf8.GetCharCount(bytes.FirstSpan); }
    ...
    private static void Validate(ReadOnlyMemory<byte> payload)
    {
        try
        {
            ...
            if (!reader.End) { throw new MessagePackSerializationException("Trailing payload data."); }
        }
