...
internal sealed class ApiOutbox : IAsyncDisposable
{
    ...
    {
        lock (SyncRoot) { _completed = true; Signal(); }
    }
    ...
    private async Task WriteAsync()
    {
        ...
        try
        {
            while (true)
            {
                ...
                catch (OperationCanceledException exception) when (deadline.IsCancellationRequested && !_stop.IsCancellationRequested)
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        finally { _signal.Dispose(); _stop.Dispose(); }
    }
}
