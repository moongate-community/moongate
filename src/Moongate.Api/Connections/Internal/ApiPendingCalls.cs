...
internal sealed class ApiPendingCalls
{
    ...
    public uint ReserveNextId()
    {
        ...
        {
            if (_lastAssignedId == uint.MaxValue) { throw new InvalidOperationException("Request identifiers are exhausted; drain and reconnect."); }
            return ++_lastAssignedId;
        }
    }

    public Task<object> RequestAsync(IApiOperationRegistration operation, object request, TimeSpan? timeout, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var timer = _clock.CreateTimer(_ => Fail(call, new TimeoutException("The API call deadline elapsed.")), null,
            ...
            lock (_outbox.SyncRoot)
            {
                ...
                if (active) { call.Deadline = timer; call.Cancellation = cancellation; }
            }
            if (!active) { timer.Dispose(); cancellation.Unregister(); return call.Source.Task; }
            ...
            lock (_outbox.SyncRoot)
            {
                ...
                var bytes = _codec.Encode(new ApiEnvelope(ApiMessageKind.Request, id, operation.Id, payload));
                call.RequestId = id;
                _pending.Add(id, call);
                if (!_outbox.TryEnqueue(new ApiOutboundFrame(bytes, id))) { throw new ApiBusyException(); }
            }
        }
    ...
    {
        lock (_outbox.SyncRoot) { if (!Remove(call)) { return; } }
        call.Release();
    ...
    private async Task WatchOutboxAsync()
    {
        ...
        catch (Exception exception) { FailAll(new IOException("The API transport failed.", exception)); return; }
