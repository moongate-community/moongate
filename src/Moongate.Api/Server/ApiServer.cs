...
public sealed class ApiServer : IApiServer
{
    ...
    {
        get { lock (_gate) { return Array.AsReadOnly(_connections.Where(connection => connection.IsConnected).Cast<IApiConnection>().ToArray()); } }
    }

    public ApiServer(IPEndPoint endpoint, ApiRegistry registry, ApiOptions options, ApiTlsOptions tls, TimeProvider timeProvider)
    {
    ...
    public Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        lock (_gate)
        {
            ...
            if (_stopCompletion is { IsCompleted: false }) { throw new InvalidOperationException("Previous API generation is still shutting down."); }
    ...
    private async Task StartCoreAsync()
    {
        ...
        var tcp = MoongateTcpServer.CreateConfigured(_endpoint, new TcpServerOptions
        {
        ...
        try
        {
            ...
            Logger.Information("API listener started at {Endpoint}: {ContractCount} contracts, {HandlerCount} handlers", tcp.Endpoint, _registry.ContractCount, _registry.HandlerCount);
        }
        catch
        {
            await tcp.DisposeAsync().ConfigureAwait(false);
            lock (_gate) { if (ReferenceEquals(_tcp, tcp)) { _tcp = null; } }
            throw;
        }
    }
    ...
    {
        await Task.CompletedTask.ConfigureAwait(ConfigureAwaitOptions.ForceYielding);
        try { await _start.ConfigureAwait(false); } catch { }
    ...
    private void ForceClose()
    {
        ApiConnection[] connections;
        MoongateTcpServer? tcp;
        lock (_gate) { connections = _connections.ToArray(); tcp = _tcp; }
    ...
    private async Task DisposeCoreAsync(Task stop)
    {
        ...
        finally { _tls.Dispose(); _slots.Dispose(); }
    }
    ...
    public ValueTask DisposeAsync()
    {
        ...
        return new ValueTask(ApiShutdown.WaitAsync(dispose, ForceClose, _options.ShutdownTimeout, _clock, Remaining));
    }
}
