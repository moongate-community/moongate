...
public sealed class ApiRegistry
{
    ...
    public void Freeze()
    {
        if (IsFrozen) { _freezeFailure?.Throw(); return; }
        IsFrozen = true;
        try { foreach (var operation in _operations.Values) { operation.ResolveHandler(); } }
        catch (Exception exception) { _freezeFailure = ExceptionDispatchInfo.Capture(exception); throw; }
    }
    ...
    public void RegisterHandler<THandler>(Func<THandler> factory) where THandler : class
    {
        ...
        var id = ValidateContract(types[0], types[1]);
        var method = typeof(ApiRegistry).GetMethod(nameof(CreateRegistration), BindingFlags.NonPublic | BindingFlags.Static)!;
        var entry = (IApiOperationRegistration)method.MakeGenericMethod(types[0], types[1], typeof(THandler)).Invoke(null, [id, factory])!;
    ...
    private static IApiOperationRegistration CreateRegistration<TRequest,TResponse,THandler>(ushort id, Func<THandler> factory)
    ...
    private static Type HandlerContract(Type handlerType)
    {
        ...
        var contracts = handlerType.GetInterfaces().Where(type => type.IsGenericType && type.GetGenericTypeDefinition() == typeof(IApiHandler<,>)).ToArray();
    ...
    private static ushort ValidateContract(Type requestType, Type responseType)
    {
        ...
        var contracts = requestType.GetInterfaces().Where(type => type.IsGenericType && type.GetGenericTypeDefinition() == typeof(IApiRequest<>)).ToArray();
