    ...
    where TRequest : IApiRequest<TResponse>
{
    ...
    { Id = id; _factory = factory; }
    public void ResolveHandler()
    {
        if (_factory is not null) { _handler = _factory() ?? throw new InvalidOperationException("Handler factory returned null."); }
    }
    public byte[] SerializeRequest(object request, int maxLength) => ApiPayloadSerializer.Serialize((TRequest)request, maxLength);
    public object DeserializeResponse(ReadOnlyMemory<byte> payload) => ApiPayloadSerializer.Deserialize<TResponse>(payload)!;
    public object DeserializeRequest(ReadOnlyMemory<byte> payload) => ApiPayloadSerializer.Deserialize<TRequest>(payload)!;
    public async ValueTask<byte[]> InvokeAsync(ApiRequestContext context, object request, int maxLength, CancellationToken token)
    {
