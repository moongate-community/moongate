...
internal sealed class ApiTlsPolicy : IDisposable
{
    ...
    public ApiTlsPolicy(ApiTlsOptions options)
    {
        ...
        if (!options.Certificate.HasPrivateKey) { throw new ArgumentException("The local TLS certificate must have a private key.", nameof(options)); }
        if (options.TrustedRoots.Count == 0) { throw new ArgumentException("At least one private trust root is required.", nameof(options)); }
        ...
        try
        {
            _certificate = new X509Certificate2(options.Certificate);
            copies.Add(_certificate);
            foreach (var root in options.TrustedRoots) { copies.Add(new X509Certificate2(root)); }
    ...
    public async ValueTask<Stream> PrepareServerAsync(Stream stream, Action<ApiPeerIdentity> authenticated, CancellationToken token)
    {
        ...
        try
        {
            await ssl.AuthenticateAsServerAsync(new SslServerAuthenticationOptions
            {
                ...
                RemoteCertificateValidationCallback = (_, certificate, _, errors) => TryAuthenticate(certificate, errors, null, out identity)
            }, token).ConfigureAwait(false);
            if (identity is null) { throw new AuthenticationException("The remote certificate did not identify an allowed peer."); }
    ...
    public async ValueTask<Stream> PrepareClientAsync(Stream stream, string targetHost, string expectedPeerId, Action<ApiPeerIdentity> authenticated, CancellationToken token)
    {
        ...
        try
        {
            await ssl.AuthenticateAsClientAsync(new SslClientAuthenticationOptions
            {
                ...
                AllowRenegotiation = false,
                AllowTlsResume = false,
                RemoteCertificateValidationCallback = (_, certificate, _, errors) => TryAuthenticate(certificate, errors, expectedPeerId, out identity)
            }, token).ConfigureAwait(false);
            if (identity is null) { throw new AuthenticationException("The remote certificate did not identify the expected peer."); }
            token.ThrowIfCancellationRequested();
            authenticated(identity);
            return ssl;
        }
    ...
    private X509ChainPolicy CreateChainPolicy(string applicationOid)
    {
        ...
        policy.ApplicationPolicy.Add(new Oid(applicationOid));
        policy.CustomTrustStore.AddRange(_roots);
        return policy;
    }

    private bool TryAuthenticate(X509Certificate? certificate, SslPolicyErrors errors, string? expectedPeerId, out ApiPeerIdentity? identity)
    {
        identity = null;
        ...
        if (expectedPeerId is not null && !string.Equals(expectedPeerId, peer.PeerId, StringComparison.Ordinal)) { return false; }
        identity = peer;
        return true;
    }
