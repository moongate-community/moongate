...
internal sealed class ApiConnectionSetup
{
    ...
    public ApiConnectionSetup(
        ApiTlsPolicy policy,
        INetFramer framer,
        Action<MoongateTcpClient, ApiPeerIdentity> configure,
        string? targetHost = null,
        string? expectedPeerId = null
    )
    {
        _policy = policy;
        _configure = configure;

        if (targetHost is null != expectedPeerId is null)
        {
    ...
    private ValueTask<Stream> PrepareAsync(
        Stream stream,
        string? targetHost,
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException("Connection setup cannot be reused.");
        }
