...
/// <summary>
///     Registers synchronous callbacks for execution on the dedicated game loop thread.
/// </summary>
public interface ITimerService : IMoongateStartupService
{
    /// <summary>
    ///     Returns a coherent snapshot of scheduling and callback measurements.
    /// </summary>
    TimerMetricsSnapshot GetMetricsSnapshot();

    /// <summary>
    ///     Registers a timer with a positive interval and optional positive first delay. Duplicate names are allowed.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    ///     Cancels every registration, including repeating callbacks currently executing.
    /// </summary>
    void UnregisterAllTimers();

    /// <summary>
    ///     Cancels a registration before its next callback is claimed; a running callback may finish.
    /// </summary>
    bool UnregisterTimer(string timerId);

    /// <summary>
    ///     Cancels every registration with the given logical name and returns the number removed.
    /// </summary>
