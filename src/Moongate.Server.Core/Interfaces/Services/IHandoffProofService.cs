...
public interface IHandoffProofService
{
    ...
    /// <summary>Binds a credential key to the full account identity, target realm and redirect key.</summary>
    byte[] Sign(ReadOnlySpan<byte> credentialKey, PendingHandoff handoff, uint authKey);

    /// <summary>Checks the full redirect identity against credentials presented to the game server.</summary>
