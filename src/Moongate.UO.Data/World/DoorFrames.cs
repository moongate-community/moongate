...
/// <summary>
/// The static graphics UO uses to draw a door frame, one set per side of a doorway.
/// <para>
/// A doorway in the map art is two of these facing each other with a gap between: a west frame and an
/// east frame two tiles apart, or a north and a south. Ported verbatim from moongatev2's door
/// generator, which took them from RunUO's <c>[doorgen</c> — they are art ids, not a rule anyone can
/// derive, so they are copied rather than reasoned about.
/// </para>
/// </summary>
public static class DoorFrames
{
    /// <summary>The 69 static graphics that are a door frame on the west side of a doorway.</summary>
    private static readonly FrozenSet<int> WestFrames = new[]
    {
    ...
    /// <summary>The 70 static graphics that are a door frame on the east side of a doorway.</summary>
    private static readonly FrozenSet<int> EastFrames = new[]
    {
    ...
    /// <summary>The 69 static graphics that are a door frame on the north side of a doorway.</summary>
    private static readonly FrozenSet<int> NorthFrames = new[]
    {
    ...
    /// <summary>The 70 static graphics that are a door frame on the south side of a doorway.</summary>
    private static readonly FrozenSet<int> SouthFrames = new[]
    {
    ...
    /// <summary>Whether the graphic is the west side of a doorway.</summary>
    public static bool IsWest(int itemId) => WestFrames.Contains(itemId);

    /// <summary>Whether the graphic is the east side of a doorway.</summary>
    public static bool IsEast(int itemId) => EastFrames.Contains(itemId);

    /// <summary>Whether the graphic is the north side of a doorway.</summary>
    public static bool IsNorth(int itemId) => NorthFrames.Contains(itemId);

    /// <summary>Whether the graphic is the south side of a doorway.</summary>
    public static bool IsSouth(int itemId) => SouthFrames.Contains(itemId);
}
