...
internal static class AdminServiceRegistration
{
    public static void Register(Container container, MoongateServerConfig config)
    {
        ...
        container.RegisterDelegate<IAdminServerInfoProvider>(resolver => new AdminServerInfoProvider(config.Mode,
            (config.Mode & Moongate.Server.Core.Types.Hosting.ServerMode.Game) != 0 ? resolver.Resolve<RealmInstance>() : null), Reuse.Singleton);
    }
}
