...
internal static class RootDirectoryInitializer
{
    public static void Initialize(string rootDirectory, string migrationsDirectory, TextWriter output)
    {
        ...
        using var initializationLock = new FileStream(
            Path.Combine(root, ".mgboot.lock"), FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None);
        ...
        config.Validate();
        CreateIfMissing(Path.Combine(root, "config/moongate.toml"),
            System.Text.Encoding.UTF8.GetBytes(TomlUtils.Serialize(config)), output);
        foreach (var catalog in source)
        {
        ...
        output.WriteLine("Configure config/moongate.toml, create the PostgreSQL databases, then apply the auth and world migrations.");
