...
/// <summary>Prepares an offline administration TLS identity while the root initializer owns its lock.</summary>
internal static class AdminCertificateSetup
{
    public static void Configure(string root, IReadOnlyList<string> hosts, TextWriter output)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                var request = new CertificateRequest("CN=Moongate Administration", key, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1);
                ...
                try
                {
                    ...
                    WriteNew(publicPath, Encoding.UTF8.GetBytes(certificate.ExportCertificatePem()),
                        UnixFileMode.UserRead | UnixFileMode.UserWrite | UnixFileMode.GroupRead | UnixFileMode.OtherRead, created);
                }
                finally
                {
    ...
    public static IReadOnlyList<string> NormalizeHosts(IReadOnlyList<string> hosts)
    {
        ...
        foreach (var value in hosts)
        {
            ...
            if (IPAddress.TryParse(name, out var address))
            {
                ...
                {
                    throw new ArgumentException("Certificate hosts must name a reachable DNS name or IP address, not a wildcard bind address.", nameof(hosts));
                }
                name = address.ToString();
            }
            else if (Uri.CheckHostName(name) != UriHostNameType.Dns || name.Contains('*'))
            {
                throw new ArgumentException("Certificate hosts must be DNS names or IP addresses without URL schemes, ports or wildcards.", nameof(hosts));
            }
            else
            {
    ...
    private static void ValidateExisting(string pfxPath, string publicPath, IReadOnlyList<string> names)
    {
        if (!File.Exists(pfxPath) || !File.Exists(publicPath))
        {
            throw new InvalidOperationException("An incomplete administration certificate pair exists. No files were overwritten.");
        }
        try
        {
            using var certificate = X509CertificateLoader.LoadPkcs12FromFile(pfxPath, "", X509KeyStorageFlags.EphemeralKeySet);
            using var publicCertificate = X509CertificateLoader.LoadCertificateFromFile(publicPath);
            if (!certificate.HasPrivateKey || !SupportsServerAuthentication(certificate) || certificate.Thumbprint != publicCertificate.Thumbprint ||
            ...
            {
                throw new InvalidOperationException("Existing administration certificates are invalid, expired, mismatched, unsuitable for server TLS or missing requested host names. No files were overwritten.");
            }
        }
        catch (CryptographicException)
        {
            throw new InvalidOperationException("Existing administration certificates cannot be read as a passwordless PFX and public PEM. No files were overwritten.");
        }
    }

    private static bool SupportsServerAuthentication(X509Certificate2 certificate)
        => certificate.Extensions.OfType<X509EnhancedKeyUsageExtension>().All(extension =>
               extension.EnhancedKeyUsages.Cast<Oid>().Any(oid => oid.Value is "1.3.6.1.5.5.7.3.1" or "2.5.29.37.0")) &&
           certificate.Extensions.OfType<X509KeyUsageExtension>().All(extension =>
