...
public sealed class WorldSaveService : IWorldSaveService
{
    ...
    public WorldSaveService(MoongatePersistenceService persistence, IGameLoopService gameLoop, ITimerService timers,
        WorldSaveOptions options, TimeProvider timeProvider)
    {
    ...
    public Task StartAsync()
    {
        lock (_gate)
        {
            ...
            _started = true;
            _logger.Information(
                "World saving started: autosave {Enabled}, interval {Interval}, backups {BackupsEnabled}, retention {Retention}",
                _options.Enabled, _options.Interval, _options.BackupsEnabled, _options.BackupRetentionCount);
    ...
    public Task StopAsync() => StopAsync(saveFinal: false);
        ...
        try
        {
            ...
            if (_options.BackupsEnabled)
            {
                ...
                await _persistence.SaveAllWithBackupAsync(
                    (capture, _) => CaptureAsync(capture, finalSave), backupDirectory, CancellationToken.None
            ...
            else
            {
                await _persistence.SaveAllAsync(
                    (capture, _) => CaptureAsync(capture, finalSave), CancellationToken.None
                ).ConfigureAwait(false);
            }
            _logger.Information("World save completed in {ElapsedMilliseconds:F2} ms; backup {BackupDirectory}",
                _timeProvider.GetElapsedTime(startedAt).TotalMilliseconds, backupDirectory);
        }
        catch (Exception exception)
        {
            _logger.Error(exception, "World save failed after {ElapsedMilliseconds:F2} ms; backup {BackupDirectory}",
                _timeProvider.GetElapsedTime(startedAt).TotalMilliseconds, backupDirectory);
            throw;
        }
    }
    ...
    private async Task CaptureAsync(Action capture, bool finalSave)
    {
        var item = new WorldSaveCaptureWorkItem(() =>
        {
            var startedAt = _timeProvider.GetTimestamp();
            capture();
            _logger.Debug("World capture completed in {ElapsedMilliseconds:F2} ms",
    ...
    private string CreateBackupDirectory()
    {
        ...
        {
            var latest = Directory.EnumerateDirectories(_options.BackupDirectory)
            ...
            {
                var previous = DateTimeOffset.ParseExact(latest.AsSpan(11, 23), "yyyyMMddTHHmmssfffffffZ",
        ...
        return Path.Combine(_options.BackupDirectory, $"world-save-{stamp}-{Guid.NewGuid():N}");
    }

    private void PruneBackups(string newestDirectory)
    {
        var completed = Directory.EnumerateDirectories(_options.BackupDirectory)
            .Where(IsCompletedBackup)
    ...
    private static bool IsCompletedBackup(string directory)
    {
        ...
        return name.Length == prefix.Length + 23 + 1 + 32 &&
               ...
               DateTimeOffset.TryParseExact(name.AsSpan(prefix.Length, 23), timestampFormat, CultureInfo.InvariantCulture,
