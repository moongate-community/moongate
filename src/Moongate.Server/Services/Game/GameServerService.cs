...
public sealed class GameServerService : IGameServerService
{
    ...
    public GameServerService(INetworkService network, IConnectionService connections, ISessionService sessions,
    ...
    public Task StartAsync()
    {
        lock (_lifecycleGate)
        {
            if (_stopping) { throw new InvalidOperationException("The game server cannot start after shutdown begins."); }
    ...
    private async Task StartCoreAsync()
    {
        ...
        catch
        {
            ...
            catch (Exception exception) { _logger.Error(exception, "Game cleanup failed after startup failure"); }
            throw;
        }
    }
    ...
    private void OnData(object? sender, NetworkDataEventArgs args)
    {
        ...
        if (PacketRegistry.Default.TryDecode(args.Data.Span, out var packet, out var opCode) &&
            _dispatcher.TryDispatch(args.Connection.SessionId, packet)) { return; }
        var packetName = PacketRegistry.Default.TryGetDescriptor(opCode, out var descriptor) ? descriptor.PacketType.Name : "Unknown";
        _logger.Warning("Rejected packet from session {SessionId}, opcode {OpCode}, name {PacketName}",
            args.Connection.SessionId, opCode, packetName);
    ...
    private async Task StopCoreAsync()
    {
        ...
        try
        {
            try { await _network.StopAsync().ConfigureAwait(false); }
