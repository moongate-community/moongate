    ...
    public X509Certificate2 Load(ApiConfig config, DirectoriesConfig directories, TimeProvider clock)
    {
        ...
        if (passwordVariable.Length > 0 && password is null)
        {
            throw new InvalidOperationException(
                $"The API certificate password environment variable '{passwordVariable}' is not set."
            );
        }
        ...
        try
        {
            ...
            _logger.Information(
                "API certificate loaded from {CertificatePath}; generated: {Generated}; SHA-256: {CertificateSha256}; expires: {ExpiresAt}",
                path,
                generated,
    ...
    private static bool Generate(ApiConfig config, string path, string? password, TimeProvider clock)
    {
        ...
        request.CertificateExtensions.Add(
            new X509EnhancedKeyUsageExtension(
                new OidCollection { new(ServerAuthenticationOid), new(ClientAuthenticationOid) },
                true
            )
        );
        ...
        foreach (var address in config.CertificateIpAddresses)
        {
