...
{
    public static async Task<RedisResult> EvaluateAsync(
    ...
    )
    {
        ...
        try
        {
            ...
            var result =
                await redis.Connection.GetDatabase().ScriptEvaluateAsync(script, keys, values).ConfigureAwait(false);
            token.ThrowIfCancellationRequested();

            return result;
        }
        catch (Exception exception) when
            (exception is RedisException or InvalidOperationException or ObjectDisposedException)
        {
