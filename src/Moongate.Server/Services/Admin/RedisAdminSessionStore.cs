...
public sealed class RedisAdminSessionStore : IAdminSessionStore
{
    ...
    public async Task<AdminAccountGate?> ReadGateAsync(Serial accountId, CancellationToken token = default)
    {
        ...
        return result is { Length: 2 } && Guid.TryParseExact((string?)result[0], "N", out var generation) &&
               (string?)result[1] is "0" or "1" ? new(generation, (string?)result[1] == "1") : null;
    }
    ...
    public async Task<AdminAccountGate> ResetGateAsync(Serial accountId, bool blocked, CancellationToken token = default)
    {
        ...
        await EvalAsync(AdminRedisScripts.ResetGate, [Gate(accountId), Index(accountId)],
            [generation.ToString("N"), blocked ? "1" : "0"], token);
    ...
    public async Task<AdminSession> IssueAsync(AdminIdentity identity, Guid generation, string tokenHash,
        TimeSpan lifetime, CancellationToken token = default)
    {
        ArgumentNullException.ThrowIfNull(identity);
        ValidateId(identity.AccountId);
        if (!Enum.IsDefined(identity.AccountType) || string.IsNullOrWhiteSpace(identity.Username) ||
            identity.Username.Length > 255 || generation == Guid.Empty || lifetime <= TimeSpan.Zero || lifetime > TimeSpan.FromDays(1))
        {
            throw new ArgumentException("Invalid administration session values.");
        }
        var expires = (long)await EvalAsync(AdminRedisScripts.Issue,
            [Gate(identity.AccountId), Index(identity.AccountId), Session(tokenHash)],
            [generation.ToString("N"), identity.AccountId.Value, identity.Username, (int)identity.AccountType,
                checked((long)Math.Ceiling(lifetime.TotalMilliseconds))], token);
        if (expires == -2)
        {
    ...
    public async Task<AdminSession?> FindAsync(string tokenHash, CancellationToken token = default)
    {
        ...
        if (values is not { Length: 5 } || !uint.TryParse((string?)values[0], out var id) || id == 0 ||
            string.IsNullOrWhiteSpace((string?)values[1]) || ((string?)values[1])!.Length > 255 ||
            !int.TryParse((string?)values[2], out var role) || !Enum.IsDefined((AccountType)role) ||
            !Guid.TryParseExact((string?)values[3], "N", out var generation) || generation == Guid.Empty ||
            !long.TryParse((string?)values[4], out var expires) || expires < 0 || expires > 253402300799999)
        {
            return null;
        }
        return new(new(new(id), (string)values[1]!, (AccountType)role), generation, DateTimeOffset.FromUnixTimeMilliseconds(expires));
    }
    ...
    private string Gate(Serial id) => _prefix + "gate:" + id.Value.ToString(CultureInfo.InvariantCulture);
    private string Index(Serial id) => _prefix + "index:" + id.Value.ToString(CultureInfo.InvariantCulture);
