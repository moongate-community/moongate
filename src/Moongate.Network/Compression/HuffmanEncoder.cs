...
/// <summary>
/// UO transport Huffman compression for outbound packets. The static code table is a
/// protocol fact shared by every UO server (256 byte codes plus a terminal code). The
/// server only ever compresses; the client decompresses, so no decoder lives here.
/// </summary>
public static class HuffmanEncoder
{
    /// <summary>UO packets may not exceed 64kb.</summary>
    public const int BufferSize = 0x10000;

    private const int MinimalCodeLength = 2;
    private const int MaximalCodeLength = 11;

    /// <summary>Fixed overhead, in bits, of the terminal code emitted once per call.</summary>
    private const int TerminalCodeLength = 4;

    /// <summary>Above this input length the output cannot fit in <see cref="BufferSize" />.</summary>
    private const int DefiniteOverflow = (BufferSize * 8 - TerminalCodeLength) / MinimalCodeLength;

    /// <summary>
    /// (bit length, bit value) pairs indexed by byte value; entry 256 (index 0x200) is the
    /// terminal code. Optimal ratio 2/8, worst 11/8.
    /// </summary>
    ...
    /// <summary>
    /// Conservative upper bound for the compressed output of <paramref name="inputLength" />
    /// bytes, or 0 when the input cannot possibly fit in <see cref="BufferSize" />.
    /// </summary>
    ...
    /// <summary>
    /// Huffman-compresses <paramref name="input" /> into <paramref name="output" />.
    /// </summary>
    /// <param name="input">Raw packet bytes.</param>
    /// <param name="output">Destination buffer; size it with <see cref="CalculateMaxCompressedSize" />.</param>
    /// <returns>Bytes written, or 0 if the input is too large or the output buffer is too small.</returns>
