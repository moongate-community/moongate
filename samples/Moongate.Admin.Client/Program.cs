...
var endpoint = new Uri(
    Environment.GetEnvironmentVariable("MOONGATE_ADMIN_ENDPOINT") ??
    throw new InvalidOperationException("MOONGATE_ADMIN_ENDPOINT is required.")
);

if (endpoint.Scheme != "https")
{
    throw new InvalidOperationException("Use a TLS administration endpoint.");
}
...
var login = await new AdminLogin.AdminLoginClient(channel).LoginAsync(
    new()
    {
        Username = Environment.GetEnvironmentVariable("MOONGATE_ADMIN_USERNAME") ??
                   throw new InvalidOperationException("MOONGATE_ADMIN_USERNAME is required."),
        Password = Environment.GetEnvironmentVariable("MOONGATE_ADMIN_PASSWORD") ??
                   throw new InvalidOperationException("MOONGATE_ADMIN_PASSWORD is required.")
    },
