...
local CATEGORIES = {
    ...
    { button_id = 7, label = "Verbal Harassment", category = "VerbalHarassment", text_gump_id = VERBAL_HARASSMENT_TEXT_GUMP_ID },
    { button_id = 8, label = "Physical Harassment", category = "PhysicalHarassment", text_gump_id = PHYSICAL_HARASSMENT_TEXT_GUMP_ID }
}

local function build_category_gump()
    local g = gump.create()
    g:resize_pic(0, 0, 9200, 420, 360)
    g:no_move()
...
local function build_text_entry_gump(category_label)
    local g = gump.create()
    g:resize_pic(0, 0, 9200, 420, 300)
    g:no_move()
    g:text(24, 20, 1152, "Help Request")
    g:text(24, 54, 0, "Category: " .. category_label)
    g:text(24, 78, 0, "Describe your issue and press Submit.")
    g:text_entry_limited(24, 112, 360, 80, 0, TEXT_ENTRY_ID, "", 400)
    g:button(24, 220, 4005, 4007, SUBMIT_BUTTON_ID)
    g:text(58, 218, 0, "Submit")
    return g
end
...
local function register_category_handler(category)
    ...
    gump.on(category.text_gump_id, SUBMIT_BUTTON_ID, function(ctx)
        ...
        local ticket_category = category.category or category.label
        return help_tickets.submit(ctx.session_id, ticket_category, message)
    end)
end
