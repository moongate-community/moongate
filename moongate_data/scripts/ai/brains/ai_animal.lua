...
local DEFAULT_PERCEPTION_RANGE = 8
...
function ai_animal.on_think(npc_serial)
    while true do
        ...
        if npc ~= nil then
            ...
            local range_fight = targeting.get_fight_range(npc_serial, DEFAULT_FIGHT_RANGE)
            ...
            if target_serial ~= nil and target_serial > 0 then
                ...
                if target ~= nil and hp_percent <= FLEE_HP_THRESHOLD then
                    fsm.set_target(npc_serial, target_serial)
                    fsm.set_action(npc_serial, fsm.actions.flee)
                    npc:set_war_mode(false)
                    combat.clear_target(npc_serial)
                    movement.flee(npc_serial, target_serial, FLEE_RANGE)
...
function ai_animal.on_death(_by_character, _context)
end
