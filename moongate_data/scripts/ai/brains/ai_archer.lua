            ...
            if target_serial ~= nil and target_serial > 0 then
                ...
                if target ~= nil then
                    ...
                    if distance < range_fight then
                        ...
                        movement.backoff(npc_serial, target_serial, range_fight)
                    elseif distance > attack_range then
                        fsm.set_action(npc_serial, fsm.actions.combat)
                        movement.combat(npc_serial, target_serial, attack_range)
                    else
...
function ai_archer.on_death(_by_character, _context)
end
