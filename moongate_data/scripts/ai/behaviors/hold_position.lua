...
function M.score(npc_serial, _ctx)
    ...
    local in_home_range = math.abs(npc.location_x - home_x) <= hold_radius and
        math.abs(npc.location_y - home_y) <= hold_radius and
        math.abs(npc.location_z - home_z) <= hold_radius

    return in_home_range and 5 or 0
end
...
behavior.register("hold_position", M)
