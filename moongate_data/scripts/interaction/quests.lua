...
local quest_journal = require("gumps.quests.quest_journal")
...
function on_quest_journal_requested(session_id, character_id)
    if session_id == nil or character_id == nil then
        return false
    end

    return quest_journal.open(session_id, character_id)
end
