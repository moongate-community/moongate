...
2. Checks exactly six `.nupkg` and six `.snupkg` files: metadata, dependencies,
...
3. Extracts the marked C# examples from each README into six temporary console apps
   outside the repository. Each app references one Moongate package directly;
   the persistence example also references MemoryPack to generate its entity serializer.
4. Restores, builds, and runs those apps, checking their output. This exercises
   ...
   the event bus, and native SkiaSharp loading.
...
An empty symbol package indicates missing PDB output. The six library projects set
...
Packability is opt-in: the shared default is `IsPackable=false`, and only the six
