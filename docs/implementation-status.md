...
| Client files | `ultima.ultima_path` checked at startup, client version logged, `tiledata.mul` loaded into `TileData` (land and item flags, heights, weights, names) and read through `ITileDataService`; map terrain and statics of every `maps.toml` map (MUL or UOP) read through `IMapService` with a block cache; every multi layout (houses, boats) from `MultiCollection.uop` or `multi.mul` read through `IMultiService` | No game system reads tiles, maps or multis yet (movement, line of sight, house placement) |
