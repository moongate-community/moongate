...
| Client files | `ultima.ultima_path` checked at startup, client version logged, `tiledata.mul` loaded into `TileData` (land and item flags, heights, weights, names) and read through `ITileDataService` | Maps, statics and multis are not loaded; no game system reads the tile flags yet |
