...
| Client files | `ultima.ultima_path` checked at startup, client version logged, `tiledata.mul` loaded into `TileData` (land and item flags, heights, weights, names) | Maps, statics and multis are not loaded; nothing reads the tile flags yet |
