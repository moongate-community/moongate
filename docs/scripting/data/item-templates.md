...
| `Stackable` | `bool?` | optional, default `null` | **Fallback only.** Whether two piles merge is read from the client's `tiledata.mul` — the `Generic` flag on the tile named by `ItemId` — so the shard agrees with what the client draws. This key is consulted only for an `ItemId` the client files do not describe, where `null` counts as not stackable. Either way both items must agree, and the merged total is capped at 60000. Enforced by `DragDropService` through `IStackableRule`. |
