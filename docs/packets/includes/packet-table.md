...
| [`0x86`](../outgoing/0x86-character-list-update.md) | Character List Update | S → C | — | The account's character list after it changed, so the client can redraw the selection screen. |
...
| [`0xA9`](../outgoing/0xa9-character-list.md) | Character List | S → C | — | The character slots followed by the starting cities, in the extended 7.0.13+ layout. |
| [`0xB9`](../outgoing/0xb9-support-features.md) | Support Features | S → C | — | Unlocks the client feature set at login, sent right before the character list. |
