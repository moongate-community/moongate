...
public sealed class MoongateTcpClientTests
{
    ...
    public async Task OnDataReceived_Throws_ClosesOnlyAffectedConnection()
    {
        ...
        await using var first = await MoongateTcpClient.ConnectAsync(new IPEndPoint(IPAddress.Loopback, server.Port));
        ...
        await using var second = await MoongateTcpClient.ConnectAsync(new IPEndPoint(IPAddress.Loopback, server.Port));
