...
public class SpanWriterTests
{
    ...
    public void Write_FixedUtf8Field_UsesEncodedByteWidth()
    {
        ...
        writer.Write("é".AsSpan(), Encoding.UTF8, fixedLength: 2);
    ...
    public void Write_FixedUtf8FieldTooNarrow_DoesNotModifyDestination()
    {
        ...
        try
        {
            writer.Write("é".AsSpan(), Encoding.UTF8, fixedLength: 1);
        }
    ...
    public void Write_FixedUtf32FieldWithoutPaddingCapacity_DoesNotModifyDestination()
    {
        ...
        try
        {
            writer.Write("\U0001F600".AsSpan(), Encoding.UTF32, fixedLength: 2);
        }
    ...
    public void WriteAttribute_LargeNormalizedValues_EncodePercentageWithoutOverflow(bool reverse)
    {
        ...
        writer.WriteAttribute(100_000_000, 50_000_000, normalize: true, reverse: reverse);
    ...
    public void WriteNullTerminated_InsufficientCapacity_IsAtomic(int encodingKind)
    {
        ...
        try
        {
            switch (encodingKind)
            {
                case 0: writer.WriteAsciiNull("AB"); break;
                case 1: writer.WriteUTF8Null("é"); break;
                case 2: writer.WriteBigUniNull("A"); break;
            }
        }
        catch (InvalidOperationException)
        {
    ...
    public void Capacity_NegativeArgument_DoesNotChangeWrittenBytes(int operation)
    {
        ...
        try
        {
            ...
            try
            {
                if (operation == 0) writer.EnsureCapacity(-1);
                else writer.Grow(-1);
            }
            catch (ArgumentOutOfRangeException)
            {
    ...
    public void Capacity_Overflow_DoesNotChangeWrittenBytes(int operation)
    {
        ...
        try
        {
            ...
            try
            {
                if (operation == 0) writer.Grow(int.MaxValue);
                else writer.EnsureRemainingCapacity(int.MaxValue);
            }
    ...
    public void Write_OwnBufferWithGrowth_PreservesCopiedBytes(bool usePacketAlias)
    {
        ...
        try
        {
            ...
            for (var i = 0; i < count; i++) writer.Write((byte)0xAB);
            if (usePacketAlias) writer.WriteBytes(writer.Span);
            else writer.Write(writer.Span);
