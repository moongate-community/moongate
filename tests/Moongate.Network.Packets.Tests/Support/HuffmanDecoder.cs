...
/// <summary>
///     Test-side decoder for the UO Huffman stream: <see cref="HuffmanEncoder" /> only compresses, like the real
///     server. It carries its own copy of the protocol's fixed code table, which doubles as an independent check
///     of the encoder's. Codes are prefix-free and MSB-first; the terminal symbol ends a packet and skips the
...
public static class HuffmanDecoder
{
    ...
    private static readonly Dictionary<(int Length, int Value), int> Codes = BuildCodes();

    /// <summary>
    ///     Decodes a stream of terminator-separated compressed packets back into raw bytes. A trailing partial code
    ///     (a chunk cut mid-packet) is ignored, so re-decoding a growing buffer is safe.
    /// </summary>
    public static byte[] Decode(ReadOnlySpan<byte> compressed)
    {
        ...
        {
            value = (value << 1) | ((compressed[bitIndex >> 3] >> (7 - (bitIndex & 7))) & 1);
            bitIndex++;
            length++;

            if (Codes.TryGetValue((length, value), out var symbol))
            {
                if (symbol == 256)
                {
                    bitIndex = (bitIndex + 7) & ~7; // skip pad bits to the byte boundary
                }
                else
                {
            ...
            {
                throw new InvalidOperationException("Huffman stream is not decodable: no code matched within 11 bits.");
            }
        }
    ...
    private static Dictionary<(int Length, int Value), int> BuildCodes()
    {
        ...
        {
            codes[(Table[symbol * 2], Table[symbol * 2 + 1])] = symbol;
        }
