...
public class ApiServerAdmissionTests
{
    ...
    {
        var handler = new GatedHandler(cooperative: false);
        await using var pair = await ApiPair.StartAsync(handler, new ApiOptions
        {
            MaxConnections = 1,
        ...
        try
        {
            ...
            for (var attempt = 0; attempt < 3; attempt++)
            {
                ...
                try { Assert.Equal(0, await raw.GetStream().ReadAsync(new byte[1], timeout.Token)); }
                catch (IOException) { }
                await Task.Delay(30);
            }
    ...
    public async Task FailedTlsSetup_ReleasesAdmissionForNextValidConnection()
    {
        ...
        {
            var error = await Record.ExceptionAsync(() => pair.Client.ConnectAsync(pair.Server.Endpoint!, "localhost", "wrong-peer"));
    ...
    private static async Task<IApiConnection> ConnectEventuallyAsync(ApiPair pair)
    {
        ...
        while (true)
        {
            try { return await pair.Client.ConnectAsync(pair.Server.Endpoint!, "localhost", "game", timeout.Token); }
