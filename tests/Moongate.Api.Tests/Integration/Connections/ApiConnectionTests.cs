...
public class ApiConnectionTests
{
    ...
    {
        await using var pair = await ApiPair.StartAsync();
        var reply = await pair.ClientConnection.RequestAsync<IncrementRequest, IncrementResponse>(new() { Value = 41 });
        Assert.Equal(42, reply.Value);
        var reverse =
            await pair.ServerConnection.RequestAsync<IncrementRequest, IncrementResponse>(new() { Value = 9 });
