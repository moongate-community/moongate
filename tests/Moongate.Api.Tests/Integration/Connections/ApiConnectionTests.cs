...
public class ApiConnectionTests
{
    ...
    public async Task TypedCalls_WorkInBothDirectionsOverMutualTls()
    {
        ...
        var reply = await pair.ClientConnection.RequestAsync<IncrementRequest, IncrementResponse>(new IncrementRequest { Value = 41 });
        Assert.Equal(42, reply.Value);
        var reverse = await pair.ServerConnection.RequestAsync<IncrementRequest, IncrementResponse>(new IncrementRequest { Value = 9 });
    ...
    {
        await using var pair = await ApiPair.StartAsync();
        var forward = Enumerable.Range(0, 16).Select(value => pair.ClientConnection.RequestAsync<IncrementRequest, IncrementResponse>(new IncrementRequest { Value = value })).ToArray();
        var reverse = Enumerable.Range(30, 16).Select(value => pair.ServerConnection.RequestAsync<IncrementRequest, IncrementResponse>(new IncrementRequest { Value = value })).ToArray();
    ...
    public async Task Disconnect_FailsPendingCallAndEventuallyRemovesSession()
    {
        ...
        while (pair.Server.Connections.Count != 0) { await Task.Delay(1, timeout.Token); }
