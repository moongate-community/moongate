...
public class ApiOutboxTests
{
    ...
    public async Task CancelQueuedRequest_ReclaimsCapacityAndPreservesWireOrder()
    {
        ...
        Assert.True(outbox.TryEnqueue(new ApiOutboundFrame([1], 1)));
        await transport.SendEntered.Task.WaitAsync(TimeSpan.FromSeconds(5));
        Assert.True(outbox.TryEnqueue(new ApiOutboundFrame([2], 2)));
        Assert.True(outbox.TryEnqueue(new ApiOutboundFrame([3], null)));
        Assert.False(outbox.TryEnqueue(new ApiOutboundFrame([4], 4)));
        Assert.False(outbox.TryRemove(1));
        Assert.True(outbox.TryRemove(2));
        Assert.False(outbox.TryRemove(3));
        Assert.True(outbox.TryEnqueue(new ApiOutboundFrame([4], 4)));
        outbox.Complete();
    ...
    public async Task StalledWrite_TimesOutAndClosesTransport()
    {
        ...
        Assert.False(outbox.TryEnqueue(new ApiOutboundFrame([2], 2)));
    }
}
