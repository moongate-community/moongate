...
internal sealed class GatedHandler : IApiHandler<IncrementRequest, IncrementResponse>
{
    ...
    public async ValueTask<IncrementResponse> HandleAsync(ApiRequestContext context, IncrementRequest request, CancellationToken cancellationToken)
    {
        Entered.Writer.TryWrite(Interlocked.Increment(ref _count));
        if (_cooperative) { await Release.Task.WaitAsync(cancellationToken); }
        else { await Release.Task; }
        if (_throwAfterRelease) { throw new InvalidOperationException("Private handler details must not reach the wire."); }
