...
internal sealed class ApiChildProcess : IAsyncDisposable
{
    ...
    public static async Task<ApiChildProcess> StartAsync(string role, object configuration)
    {
        ...
        var child = new ApiChildProcess(Process.Start(start) ?? throw new InvalidOperationException("Could not start test host."));
        try
        {
    ...
    {
        try { return await _lines.Reader.ReadAsync().AsTask().WaitAsync(TimeSpan.FromSeconds(15)); }
        catch (ChannelClosedException) { throw new InvalidOperationException($"Test host exited: {await _stderr}"); }
    }
    ...
    public async Task ExpectExitAsync()
    {
        ...
        if (_process.ExitCode != 0) { throw new InvalidOperationException($"Test host failed: {await _stderr}"); }
    }
    private async Task DrainOutputAsync()
    {
        try
        {
            while (await _process.StandardOutput.ReadLineAsync() is { } line) { await _lines.Writer.WriteAsync(line); }
        }
        finally { _lines.Writer.TryComplete(); }
    }
    public async ValueTask DisposeAsync()
    {
        ...
        try
        {
            ...
            {
                try { await SendAsync("STOP"); _process.StandardInput.Close(); }
                catch (IOException) { }
