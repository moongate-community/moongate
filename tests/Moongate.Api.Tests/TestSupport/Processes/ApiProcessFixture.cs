...
internal sealed class ApiProcessFixture : IAsyncDisposable
{
    ...
    public static async Task<ApiProcessFixture> StartAsync()
    {
        ...
        try
        {
            fixture._login = await ApiChildProcess.StartAsync("login", new { Certificate = Export(login), Root = root, Peers = peers, Permissions = permissions });
            fixture._loginPort = ParsePort(await fixture._login.ReadLineAsync());
            fixture._game = await ApiChildProcess.StartAsync("game", new { Certificate = Export(game), Root = root, Peers = peers, Permissions = permissions });
            fixture._gamePort = ParsePort(await fixture._game.ReadLineAsync());
            return fixture;
        }
        catch { await fixture.DisposeAsync(); throw; }
    }
    public async Task<int> CallIncrementAsync(int value) => int.Parse(await CallAsync(_loginPort, _loginFingerprint, value));
    public async Task<int> CallGameIncrementAsync(int value) => int.Parse(await CallAsync(_gamePort, _gameFingerprint, value));
    public Task<string> CallForbiddenAsync() => CallAsync(_gamePort, _gameFingerprint, 41, denied: true);
    public Task<string> LoseReplyAndReconnectAsync() => CallAsync(_gamePort, _gameFingerprint, -1, reconnect: true);
    ...
    private async Task<string> CallAsync(int port, string fingerprint, int value, bool denied = false, bool reconnect = false)
    {
        await using var client = await ApiChildProcess.StartAsync("client", new
        {
    ...
    private static string Export(X509Certificate2 certificate) => Convert.ToBase64String(certificate.Export(X509ContentType.Pkcs12));
    private static int ParsePort(string ready)
    {
        if (!ready.StartsWith("READY ", StringComparison.Ordinal)) { throw new InvalidDataException("Test host did not become ready."); }
        return int.Parse(ready["READY ".Length..]);
    }
    public async ValueTask DisposeAsync()
    {
        try { if (_login is not null) { await _login.DisposeAsync(); } }
        finally { if (_game is not null) { await _game.DisposeAsync(); } }
    }
}
