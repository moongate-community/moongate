...
internal sealed class StubApiConnection : IApiConnection
{
    ...
    public StubApiConnection(params ushort[] permissions) { Peer = new ApiPeerIdentity("test-peer", permissions); }
    public Task<TResponse> RequestAsync<TRequest, TResponse>(TRequest request, TimeSpan? timeout = null, CancellationToken cancellationToken = default) where TRequest : IApiRequest<TResponse>
