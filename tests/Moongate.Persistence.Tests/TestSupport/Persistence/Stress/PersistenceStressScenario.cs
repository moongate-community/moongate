...
internal static class PersistenceStressScenario
{
    public static async Task<StressPhaseReport> RunAsync(
        PostgreSqlTestDatabase database, int sessions, int concurrency, int seconds, int thinkMilliseconds, int? operationsPerSession = null)
    {
        ...
        var states = Enumerable.Range(0, sessions).Select(number => new SyntheticSessionEntity
        {
        ...
        long commits = 0, rollbacks = 0;
        double elapsed, cpuSeconds;
        DateTimeOffset startedAtUtc, finishedAtUtc;
        long allocated, workingSet;
        ...
        using (var container = CreateOwner(database, concurrency, true))
        {
            ...
            await Parallel.ForEachAsync(states, new ParallelOptions { MaxDegreeOfParallelism = concurrency },
            ...
            allocated = GC.GetTotalAllocatedBytes(false) - allocatedBefore;
            process.Refresh();
            ...
            {
                await start.Task;
                for (var step = 0; operationsPerSession is { } limit ? step < limit : Stopwatch.GetElapsedTime(started).TotalSeconds < seconds; step++)
                {
                    var operation = step % 10;
                    var rollback = operation == 9 && (step / 10) % 2 == 1;
                    var name = operation < 5 ? "read" : operation < 7 ? "query" : operation < 9 ? "update" : rollback ? "rollback" : "commit";
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                try
                                {
                                    await owner.ExecuteInTransactionAsync(PersistenceDatabaseTarget.Realm, async transaction =>
                                    {
                                        ...
                                        await transaction.GetDataAccess<SyntheticWriteEntity>().UpsertAsync(entry, timeout.Token);
                                        Require(entry.Id.IsValid, "Unassigned transaction identity");
                                        await transaction.GetDataAccess<SyntheticSessionEntity>().UpsertAsync(next, timeout.Token);
                                        if (rollback)
                                        {
                    ...
                    {
                        var category = exception is OperationCanceledException or TimeoutException || timeout.IsCancellationRequested
                            ? "timeout" : exception.GetType().Name;
        ...
        try
        {
            ...
            Require(entries.Select(entry => entry.Id).Distinct().Count() == entries.Count, "Duplicate transaction identities");
            var bySession = entries.ToLookup(entry => entry.SessionNumber);
            for (var number = 0; number < sessions; number++)
            {
                Require(bySession[number].Select(entry => entry.Revision).Order().SequenceEqual(committedRevisions[number].Order()),
        ...
        return new(startedAtUtc, finishedAtUtc, sessions, concurrency, elapsed, reports.Values.Sum(value => value.Successes) / elapsed,
            commits, rollbacks, allocated, cpuSeconds, workingSet, verified, reports, Snapshot(admission), errors);
    }
    ...
    private static Container CreateOwner(PostgreSqlTestDatabase database, int concurrency, bool synchronize)
    {
        ...
        container.RegisterMoongatePersistence(new([new(PersistenceDatabaseTarget.Realm, connection.ConnectionString)], synchronize))
    ...
    private static string Payload(int session, long revision) => $"session:{session};revision:{revision};{new string('x', 256)}";

    private static void AssertState(SyntheticSessionEntity expected, SyntheticSessionEntity? actual)
        => Require(actual is not null && actual.Id == expected.Id && actual.SessionNumber == expected.SessionNumber &&
                   actual.Revision == expected.Revision && actual.Payload == expected.Payload, "Session data mismatch");
