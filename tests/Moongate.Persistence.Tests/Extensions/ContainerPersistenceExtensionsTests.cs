...
public sealed class ContainerPersistenceExtensionsTests
{
    ...
    public async Task AuthAndWorld_WithoutModules_RouteToSeparateDatabasesAndCaptureSnapshots()
    {
        ...
        container.AddPersistenceWorld<CharacterEntity>(() => [source], value => new CharacterEntity { Id = value.Id, Name = value.Name });
        await using var owner = container.Resolve<MoongatePersistenceService>();

        await owner.InitializeAsync();
        await container.Resolve<IDataAccess<AccountsSharedEntity>>().UpsertAsync(new AccountsSharedEntity { Id = new Serial(1) });
        ...
        Assert.False(await auth.ScalarAsync<bool>("SELECT EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'plugin_characters')"));
        Assert.False(await world.ScalarAsync<bool>("SELECT EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'plugin_shared')"));
    ...
    public async Task AuthSnapshot_DoesNotActivateWorldAndRejectsDuplicateTargetRegistration()
    {
        ...
        container.RegisterMoongatePersistence(new PostgreSqlPersistenceOptions([
            new(PersistenceDatabaseTarget.Accounts, database.ConnectionString),
            new(PersistenceDatabaseTarget.Realm, () => throw new InvalidOperationException("World must stay inactive."))
        ], true));
        container.AddPersistenceAuth<AccountsSharedEntity>(() => [new AccountsSharedEntity { Id = new Serial(1) }],
