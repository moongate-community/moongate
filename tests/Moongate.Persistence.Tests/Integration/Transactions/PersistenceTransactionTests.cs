...
public sealed class PersistenceTransactionTests
{
    ...
    public PersistenceTransactionTests(PostgreSqlFixture postgres) { _postgres = postgres; }
    ...
    public async Task ExecuteInTransactionAsync_TwoSchemas_ReadsOwnWritesAndCommitsAtomically()
    {
        ...
        await owner.ExecuteInTransactionAsync(PersistenceDatabaseTarget.Realm, async tx =>
        {
            ...
            await tx.GetDataAccess<InventoryEntity>().UpsertAsync(new InventoryEntity { Id = new Serial(2), Balance = 5 });
    ...
    public async Task ExecuteInTransactionAsync_FailureOrCaughtMisuse_RollsBackWholeGroup(string failure)
    {
        ...
        await Assert.ThrowsAnyAsync<Exception>(() => owner.ExecuteInTransactionAsync(PersistenceDatabaseTarget.Realm, async tx =>
        {
            await tx.GetDataAccess<CharacterEntity>().UpsertAsync(new CharacterEntity { Id = new Serial(1), Name = "rollback" });
            await tx.GetDataAccess<InventoryEntity>().UpsertAsync(new InventoryEntity { Id = new Serial(2), Balance = 5 });
            if (failure == "callback")
            {
            ...
            {
                try { await tx.GetDataAccess<CharacterEntity>().UpsertAsync(new CharacterEntity { Id = new Serial(3), Name = new string('x', 200) }); }
            ...
            if (failure == "nested")
            {
                await Assert.ThrowsAsync<InvalidOperationException>(() => owner.ExecuteInTransactionAsync(PersistenceDatabaseTarget.Realm, _ => Task.CompletedTask));
            }
    ...
    public async Task ExecuteInTransactionAsync_ConcurrentOrUnawaitedRead_DrainsAndPoisons(bool unawaited)
    {
        ...
        await using (var command = new NpgsqlCommand("LOCK plugin_characters.characters IN ACCESS EXCLUSIVE MODE", blocker, held))
        {
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task ExecuteInTransactionAsync_DifferentTarget_RejectionPoisonsButIndependentOperationsProceed(bool standalone)
    {
        ...
        var pending = owner.ExecuteInTransactionAsync(PersistenceDatabaseTarget.Realm, async tx =>
        {
        ...
        try
        {
            ...
            await accountStore.UpsertAsync(new AccountsSharedEntity { Id = new Serial(1) }).WaitAsync(TimeSpan.FromSeconds(10));
    ...
    public async Task ExecuteInTransactionAsync_ClosingOwnerStillPoisonsCaughtStandaloneReentry()
    {
        ...
        release.SetResult();
        try { await Assert.ThrowsAsync<InvalidOperationException>(() => transaction); }
        finally { await close; }
        Assert.Equal(0L, await database.ScalarAsync<long>("SELECT count(*) FROM plugin_characters.characters"));
    }
    ...
    public async Task ExecuteInTransactionAsync_RollbackConnectionFails_PreservesOriginalCallbackErrorWithoutRetry()
    {
        ...
        var calls = 0;
        var observed = await Assert.ThrowsAsync<InvalidOperationException>(() => owner.ExecuteInTransactionAsync(PersistenceDatabaseTarget.Realm, async tx =>
        {
            calls++;
            await tx.GetDataAccess<CharacterEntity>().UpsertAsync(new CharacterEntity { Id = new Serial(1) });
            Assert.True(await database.ScalarAsync<bool>("SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = current_database() AND state = 'idle in transaction' AND pid <> pg_backend_pid()"));
            throw original;
        }));
    ...
    public async Task ExecuteInTransactionAsync_OuterCancellation_CancelsScopedIoWithoutAnExplicitChildToken()
    {
        ...
        try
        {
            ...
            Assert.True(error is OperationCanceledException || error.InnerException is OperationCanceledException,
