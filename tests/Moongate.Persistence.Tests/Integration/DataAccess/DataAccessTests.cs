...
public sealed class DataAccessTests
{
    ...
    public async Task UpsertAsync_DetachedReadsAndFullRangeIds_PreservesCommittedValues()
    {
        ...
        foreach (var value in new uint[] { 1, 0x80000000, uint.MaxValue })
        {
            ...
            await store.UpsertAsync(new CharacterEntity { Id = id, Name = "before" });
            var copy = await store.GetByIdAsync(id);
            copy!.Name = "not saved";
            Assert.Equal("before", (await store.GetByIdAsync(id))!.Name);
            await store.UpsertAsync(new CharacterEntity { Id = id, Name = "after" });
    ...
    public async Task QueryAsync_SerialExpressionsAndPaging_FiltersInDatabase()
    {
        ...
        owner.RegisterEntity<InventoryEntity>();
        await owner.InitializeAsync();
        foreach (uint value in new uint[] { 3, 1, 2 })
        {
            await store.UpsertAsync(new CharacterEntity { Id = new Serial(value), Name = value == 2 ? "O'Brien" : "other" });
        }
        ...
        Assert.Equal(new Serial(2), Assert.Single(await store.QueryAsync(e => e.Name == "O'Brien")).Id);
        Assert.Equal(new Serial(2), Assert.Single(await store.QueryAsync(e => e.Id > Serial.Zero, 1, 1)).Id);
    ...
    public async Task Operations_InvalidArguments_RejectWithoutChangingData()
    {
        ...
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() => store.UpsertAsync(new CharacterEntity()));
