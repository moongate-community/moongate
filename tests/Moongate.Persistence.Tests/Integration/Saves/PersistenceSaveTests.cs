...
public sealed class PersistenceSaveTests
{
    ...
    public PersistenceSaveTests(PostgreSqlFixture postgres)
    {
        _postgres = postgres;
    }
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => owner.SaveAllAsync(
                (capture, _) =>
                {
                    ...
                    {
                        return Task.Run(capture, CancellationToken.None);
                    }
                }
            )
        );
    ...
    public async Task SaveAllAsync_CaptureThenDelete_OrdersMutationsWithoutResurrection()
    {
        await using var database = await _postgres.CreateDatabaseAsync();
        await using var owner = FacadeFixture.Create(database);
        var live = new CharacterEntity { Id = new(1), Name = "snapshot" };
        var store = owner.RegisterEntity<CharacterEntity>(
            () => [live],
    ...
    [Theory, InlineData("success", false), InlineData("cancellation", false), InlineData("failure", true)]
    public async Task SaveAllAsync_DispatcherExitsDuringActiveCapture_DrainsBeforeMutationAndDisposal(
        string outcome,
        bool blockSnapshot
    )
    {
        ...
        var live = new CharacterEntity { Id = new(1), Name = "must not be saved" };
        var store = owner.RegisterEntity<CharacterEntity>(
            () =>
            {
                if (!blockSnapshot)
            ...
            entity =>
            {
                ...
                return new() { Id = entity.Id, Name = entity.Name };
            }
        );
        owner.RegisterEntity<InventoryEntity>();
        await owner.InitializeAsync();
        ...
        try
        {
            ...
            mutation = store.UpsertAsync(new() { Id = new(2), Name = "independent" });
        ...
        finally
        {
            ...
            {
                await captureTask.ConfigureAwait(
        ...
        Assert.Equal(
            "independent",
            await database.ScalarAsync<string>("SELECT name FROM plugin_characters.characters WHERE id = 2")
        );
    }
    ...
    public async Task SaveAllAsync_ExplicitTransactionAlreadyAdmitted_WaitsBeforeEnumeratingSource()
    {
        ...
        await using var owner = FacadeFixture.Create(database);
        var captures = 0;
        var live = new CharacterEntity { Id = new(1), Name = "old" };
        var store = owner.RegisterEntity<CharacterEntity>(
            () =>
            {
                captures++;
        ...
        var transaction = owner.ExecuteInTransactionAsync(
            ...
            async tx =>
            {
                await tx.GetDataAccess<CharacterEntity>().UpsertAsync(new() { Id = live.Id, Name = "new" });
                entered.SetResult();
                await release.Task;
            }
        );
    ...
    public async Task SaveAllAsync_InvalidCapture_WritesNothingForTarget(string failure)
    {
        await using var database = await _postgres.CreateDatabaseAsync();
        await using var owner = FacadeFixture.Create(database);
        var live = new CharacterEntity { Id = new(1), Name = "live" };
        var store = owner.RegisterEntity<CharacterEntity>(
            () => failure == "duplicate" ? [live, live] : [live],
            e => failure switch
            {
                "zero"       => new(),
                "null"       => null!,
                "same"       => e,
                "changed_id" => new() { Id = new(2) },
                _            => new() { Id = e.Id, Name = e.Name }
            }
        );
    ...
    public async Task SaveAllAsync_LaterCaptureInvalid_PerformsNoSqlWritesEvenBeforeRollback()
    {
        ...
        owner.RegisterEntity<InventoryEntity>(() => [new() { Id = new(2) }], e => e);
        await owner.InitializeAsync();
    ...
    public async Task SaveAllAsync_LaterTargetCaptureFails_KeepsEarlierTargetCommit()
    {
        ...
        await using var owner = new MoongatePersistenceService(
            new(
                [
                    new(PersistenceDatabaseTarget.Accounts, accounts.ConnectionString),
                    new(PersistenceDatabaseTarget.Realm, realm.ConnectionString)
                ],
                true
            )
        );
        ...
        var committed = Assert.Single(
            logs.Events,
            e => e.MessageTemplate.Text.StartsWith("PostgreSQL snapshot committed", StringComparison.Ordinal)
        );
        Assert.Equal("Accounts", committed.Properties["Target"].ToString());
        Assert.Equal("1", committed.Properties["EntityCount"].ToString());
        Assert.DoesNotContain(
            logs.Events,
            e => e.MessageTemplate.Text.StartsWith("PostgreSQL world save completed", StringComparison.Ordinal)
        );
    }
    ...
    public async Task SaveAllAsync_NestedSnapshotAndAbsentSource_PreservesDetachedValuesAndExistingRows()
    {
        ...
        var live = new SnapshotEntity { Id = new(1), Payload = [10, 20] };
        var present = true;
        var store = owner.RegisterEntity<SnapshotEntity>(
            () => present ? [live] : [],
    ...
    public async Task SaveAllAsync_SnapshotMutatesLiveIdentity_RejectsChangedIdentity()
    {
        await using var database = await _postgres.CreateDatabaseAsync();
        await using var owner = FacadeFixture.Create(database);
        var live = new CharacterEntity { Id = new(1) };
        var store = owner.RegisterEntity<CharacterEntity>(
            () => [live],
            e =>
            {
