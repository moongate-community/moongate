...
public sealed class MigrationReadinessTests
{
    ...
    public async Task InitializeAsync_AppliedDataMigration_ValidatesChecksum()
    {
        ...
        await db.ExecuteAsync("CREATE SCHEMA moongate_migrations; CREATE TABLE moongate_migrations.history (target text, component text, script text, checksum text);" +
    ...
    {
        using var files = new MigrationFiles();
        var options = new PostgreSqlPersistenceOptions([new(PersistenceDatabaseTarget.Accounts, () => throw new Exception("unused"))],
    ...
    {
        var options = new PostgreSqlPersistenceOptions([new(PersistenceDatabaseTarget.Realm, db.ConnectionString), new(PersistenceDatabaseTarget.Accounts, () => throw new Exception("unused"))],
            migrationCatalogFactory: target => MigrationCatalog.Load(files.Core, null, target == PersistenceDatabaseTarget.Accounts ? MigrationTarget.Auth : MigrationTarget.World));
