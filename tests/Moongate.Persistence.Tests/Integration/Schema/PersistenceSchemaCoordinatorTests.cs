...
public sealed class PersistenceSchemaCoordinatorTests
{
    ...
    public async Task SynchronizeAsync_ExplicitRenameAdditionAndWidening_RetainsSeededRows()
    {
        ...
        Assert.Equal(
            before,
            await database.ScalarAsync<string>("SELECT display_name FROM plugin_upgrade.characters WHERE id = 1")
        );
        Assert.Equal(
            1,
            await database.ScalarAsync<int>("SELECT level FROM plugin_upgrade.characters WHERE id = 1")
        );
    ...
    public async Task SynchronizeAsync_FreshThenReopenedModel_IsStableAndAutoSyncStaysDisabled()
    {
        ...
        Assert.Equal(
            "seeded",
            await database.ScalarAsync<string>("SELECT name FROM plugin_characters.characters WHERE id = 1")
        );
    }
    ...
    public async Task SynchronizeAsync_TwoSchemasMayUseTheSameTableAndIdNames()
    {
        ...
        await coordinator.SynchronizeAsync();
        var orm = coordinator.GetDatabase(PersistenceDatabaseTarget.Realm).Orm;
        await orm.Insert(new CharacterSharedEntity { Id = new(7), Name = "character" }).ExecuteAffrowsAsync();
        await orm.Insert(new InventorySharedEntity { Id = new(7), Balance = 42 }).ExecuteAffrowsAsync();

        Assert.Equal(
            "character",
            await database.ScalarAsync<string>("SELECT name FROM plugin_characters.entities WHERE id = 7")
        );
        Assert.Equal(
            42,
            await database.ScalarAsync<int>("SELECT balance FROM plugin_inventory.entities WHERE id = 7")
        );
    }
    ...
    )
    {
        ...
        return new(options, CreateRegistry(modules));
    }
