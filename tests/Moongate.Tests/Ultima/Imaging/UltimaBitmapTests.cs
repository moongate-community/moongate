...
public class UltimaBitmapTests
{
    ...
    public void ToImage_Argb1555Pixels_PreservesColorsTransparencyAndRows()
    {
        ...
        Assert.Equal(new SKColor(255, 0, 0), image.GetPixel(0, 0));
        Assert.Equal(new SKColor(0, 255, 0), image.GetPixel(1, 0));
        Assert.Equal(new SKColor(0, 0, 255), image.GetPixel(2, 0));
        Assert.Equal(new SKColor(0, 0, 0, 0), image.GetPixel(0, 1));
        Assert.Equal(new SKColor(0, 0, 0), image.GetPixel(1, 1));
        Assert.Equal(new SKColor(255, 255, 255), image.GetPixel(2, 1));
    }
    ...
    public void ToImage_OpaqueRgb555_PreservesColorsWithoutAlphaBit()
    {
        ...
        using var image = bitmap.ToImage(opaque: true);

        Assert.Equal(new SKColor(255, 0, 0), image.GetPixel(0, 0));
        Assert.Equal(new SKColor(0, 0, 0), image.GetPixel(1, 0));
    }
    ...
    public void Save_Png_RoundTripsOpaqueBlackAndTransparentPixels()
    {
        ...
        try
        {
            ...
            Assert.Equal(new SKColor(0, 0, 0), decoded.GetPixel(1, 0));
            Assert.Equal(new SKColor(0, 0, 0, 0), decoded.GetPixel(2, 0));
    ...
    public void FromImage_RgbaWithPaddedRows_PreservesChannelOrderAndAlphaThreshold()
    {
        ...
        using var source = new SKBitmap(info, rowBytes: 20);
        byte[] pixels =
        [
    ...
    {
        using var source = new SKBitmap(new SKImageInfo(1, 1, SKColorType.Bgra8888, SKAlphaType.Premul));
    ...
    {
        using var source = new SKBitmap(new SKImageInfo(1, 1, SKColorType.Bgra8888, SKAlphaType.Opaque));
