...
public sealed class LuaCSharpBehaviourTests
{
    ...
    public void ACoroutineCreatedFromLua_DoesNotInheritTheHook()
    {
        ...
        Sync(state.DoStringAsync(Loop, "inline"));
        var inlineHits = hits;
        hits = 0;
        Sync(state.DoStringAsync($"local f = coroutine.wrap(function() {Loop} end) return f()", "viaCoroutine"));
    ...
    public void AHookThatThrows_StopsFiringForTheRestOfTheStatesLife()
    {
            ...
            => state.SetHook(
                new(
                    ...
                    (context, _) =>
                    {
                        ...
                        if (hits % 3 == 0)
                        {
                            throw new LuaRuntimeException(context.State, new LuaValue("budget exceeded"));
                        }
        ...
        Assert.Throws<LuaRuntimeException>(() => Sync(state.DoStringAsync("while true do end", "first")));
        Assert.Equal(3, hits);
        ...
        Sync(state.DoStringAsync("local n = 0 for i = 1, 5000 do n = n + i end return n", "second"));

        Assert.Equal(3, hits);

        InstallThrowingHook();
        Sync(state.DoStringAsync("local n = 0 for i = 1, 5000 do n = n + i end return n", "third"));
    ...
    public void ASuspendedCoroutine_KeepsTheTokenOfItsFirstResume()
    {
        ...
        var yielded = state.CreateCoroutine(job);
        ...
        var other = state.CreateCoroutine(job);
    ...
    public void Coroutine_YieldCompletesSynchronouslyAndLeavesValuesOnTheStack()
    {
        ...
        var coroutine = state.CreateCoroutine(state.Environment["job"].Read<LuaFunction>(), true);
        var stack = new LuaStack(16);

        var first = Sync(coroutine.ResumeAsync(stack));
        ...
        stack.Clear();
        stack.Push(new("resumed"));
        var second = Sync(coroutine.ResumeAsync(stack));
    ...
    public void CountHook_InterruptsABareInfiniteLoop()
    {
        ...
        var exception =
            Assert.Throws<LuaRuntimeException>(() => Sync(state.DoStringAsync("while true do end", "probe")));
    ...
    public void LoadedModules_EvictionMakesRequireReloadTheModule()
    {
        ...
        var first = Sync(state.DoStringAsync("return require('m').n", "probe"))[0].Read<double>();
        var cached = Sync(state.DoStringAsync("return require('m').n", "probe"))[0].Read<double>();
        state.LoadedModules["m"] = LuaValue.Nil;
        var reloaded = Sync(state.DoStringAsync("return require('m').n", "probe"))[0].Read<double>();
    ...
    public void OpenBasicLibrary_DefinesDofileAndLoadfile()
    {
        ...
        var result = Sync(state.DoStringAsync("return type(dofile), type(loadfile), type(rawset)", "probe"));
    ...
    public void OpenModuleLibrary_InstallsTwoSearchers_AndTheSecondHonoursPackagePath()
    {
        ...
        Assert.Equal(2, Sync(state.DoStringAsync("return #package.searchers", "probe"))[0].Read<double>());
    ...
    public void ProtectedCoroutine_ReportsAnErrorAsAValueInsteadOfThrowing()
    {
        ...
        state.OpenBasicLibrary();
        state.OpenCoroutineLibrary();
        Sync(state.DoStringAsync("function boom() error('kaboom') end", "scripts/boom.lua"));
        var coroutine = state.CreateCoroutine(state.Environment["boom"].Read<LuaFunction>(), true);
        var stack = new LuaStack(8);

        var count = Sync(coroutine.ResumeAsync(stack));
    ...
    public void ReadOnlyProxy_BlocksWritesToExistingAndNewKeys()
    {
        ...
        meta["__index"] = new(hidden);
        meta["__newindex"] = new LuaFunction(
            "ro",
            (context, _) =>
                throw new LuaRuntimeException(context.State, new LuaValue("read-only"))
        );
        meta["__metatable"] = new("locked");
        var proxy = new LuaTable { Metatable = meta };
        state.Environment["m"] = proxy;

        Assert.Equal(42, Sync(state.DoStringAsync("return m.LIMIT", "p"))[0].Read<double>());
        Assert.Throws<LuaRuntimeException>(() => Sync(state.DoStringAsync("m.LIMIT = 1", "p")));
        Assert.Throws<LuaRuntimeException>(() => Sync(state.DoStringAsync("m.NEW = 1", "p")));
        Assert.Throws<LuaRuntimeException>(() => Sync(state.DoStringAsync("setmetatable(m, {})", "p")));
    }
    ...
    public void RuntimeError_CarriesTheChunkNameAndLine()
    {
        ...
        var exception = Assert.Throws<LuaRuntimeException>(() => Sync(state.ExecuteAsync(closure)));
    ...
    public void SelectiveLibraries_LeaveIoAndOsUndefined()
    {
        ...
        var result = Sync(state.DoStringAsync("return tostring(io), tostring(os), string.upper('ok')", "probe"));
    ...
    public void UnprotectedCoroutine_ThrowsWithTheChunkNameAndLine()
    {
        ...
        state.OpenCoroutineLibrary();
        Sync(state.DoStringAsync("function boom() error('kaboom') end", "scripts/boom.lua"));
        var coroutine = state.CreateCoroutine(state.Environment["boom"].Read<LuaFunction>());

        var exception = Assert.Throws<LuaRuntimeException>(() => Sync(coroutine.ResumeAsync(new LuaStack(8))));
