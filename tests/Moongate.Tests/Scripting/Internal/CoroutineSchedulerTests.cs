...
public sealed class CoroutineSchedulerTests : IDisposable
{
    ...
    public CoroutineSchedulerTests()
    {
        ...
        _budget.Install();
        _scheduler = new(_state, _timers, _budget, _ownership, _errors.Add, () => "test.lua");
        SyncValueTask.Run(_state.DoStringAsync("function wait(s) return coroutine.yield('wait', s) end", "prelude"));
    }
    ...
    {
        SyncValueTask.Run(_state.DoStringAsync("function add(a, b) return a + b end", "add.lua"));
    ...
    {
        SyncValueTask.Run(_state.DoStringAsync($"function {name}() {body} end", name + ".lua"));
