...
/// <summary>
/// Test-side decoder for the UO transport Huffman stream (<see cref="HuffmanEncoder" /> only
/// compresses, mirroring the real server). Carries its own copy of the protocol's fixed code
/// table, which doubles as an independent cross-check of the encoder's. Codes are prefix-free
/// and MSB-first; the terminal symbol ends a packet and skips the padding to the byte boundary.
/// </summary>
public static class HuffmanDecoder
{
    ...
    /// partial code (chunk cut mid-packet) is ignored, so re-decoding a growing buffer is safe.
    /// </summary>
    public static byte[] Decode(ReadOnlySpan<byte> compressed)
    {
        ...
        while (bitIndex < totalBits)
        {
            ...
            if (_codes.TryGetValue((length, value), out var symbol))
            {
                if (symbol == 256)
                {
