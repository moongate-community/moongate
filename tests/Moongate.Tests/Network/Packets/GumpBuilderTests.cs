...
public class GumpBuilderTests
{
        ...
        Assert.Multiple(
            () =>
            {
                Assert.That(
                    builder.BuildLayout(),
                    Is.EqualTo(
                        "{ text 25 40 1152 0 } { htmlgump 30 70 200 100 1 1 1 }"
                    )
                );
    ...
    public void ButtonMethods_ShouldWriteExpectedButtonTypes()
    {
        ...
        Assert.That(
            layout,
            Is.EqualTo(
                "{ button 50 80 4005 4007 1 0 1 } { button 90 80 4011 4013 0 2 0 }"
            )
        );
    }
    ...
    public void CheckBox_ShouldWriteSwitchIdAndInitialState()
    {
        ...
        Assert.That(
            layout,
            Is.EqualTo(
                "{ checkbox 20 25 210 211 0 42 } { checkbox 20 45 210 211 1 43 }"
            )
        );
    }
    ...
    public void ToCompressedPacket_ShouldMapBuilderState()
    {
        var packet = new GumpBuilder()
                     .NoMove()
                     .NoClose()
                     .HtmlLocalized(30, 70, 200, 100, "Body", true, false)
