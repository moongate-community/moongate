...
public sealed class AdminAuditTests
{
        ...
        try
        {
            ...
            foreach (var operation in new[] { "/Login", "/Logout" })
            {
                var entry = Assert.Single(
                    audits,
                    e => e.Properties["Operation"].ToString().Contains(operation, StringComparison.Ordinal)
                );
            ...
            Assert.DoesNotContain(
                sink.Events,
                e => (e.RenderMessage() + e.Exception).Contains(password, StringComparison.Ordinal) ||
    ...
    public async Task CreateAccount_ProviderError_DoesNotLogCredentialMaterial()
    {
        ...
        try
        {
            ...
            var errors = sink.Events.Where(e => e.Level == LogEventLevel.Error).ToArray();
