...
public sealed class AccountAdminAccessServiceTests
{
    ...
    public async Task LoginAsync_InvalidAccountState_DoesNotIssueOrRecordLogin(string state)
    {
        ...
        var result = await fixture.Accounts.Service.CreateAccountAsync(new AccountCreateOptions
        {
        ...
        Assert.Null(await fixture.Authority.LoginAsync(state == "unknown" ? "missing" : "admin",
    ...
    {
        await using var fixture = await AccountAdminFixture.CreateAsync();
        var result = await fixture.Accounts.Service.CreateAccountAsync("admin", fixture.Accounts.Password, AccountType.Administrator);
        ...
        Assert.Equal(AccountType.Administrator, (await fixture.Accounts.Accounts.GetByIdAsync(result.Account!.Id))!.AccountType);
    ...
    {
        await using var fixture = await AccountAdminFixture.CreateAsync();
        var created = await fixture.Accounts.Service.CreateAccountAsync(new AccountCreateOptions
        {
            Username = "admin", Password = fixture.Accounts.Password, AccountType = AccountType.Administrator, CanAccessApi = true
        });
        var account = created.Account!;
        var entered = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var release = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        fixture.Store.BeforeIssue = async () => { entered.TrySetResult(); await release.Task; };
        ...
        var mutation = change switch
        {
            ...
            _ => fixture.PeerAuthority.UpdateAccessAsync(account.Id, new AccountAccessOptions
            {
        ...
        var next = await fixture.Authority.LoginAsync("admin", change == "password" ? newPassword : fixture.Accounts.Password);
    ...
    public async Task UpdateAccessAsync_FailsAroundCommit_RemainsBlockedUntilAuthoritativeRecovery(bool afterCommit)
    {
        ...
        await Assert.ThrowsAsync<AdminDependencyUnavailableException>(() => fixture.Authority.UpdateAccessAsync(created.Account!.Id,
            new AccountAccessOptions { AccountType = AccountType.Regular, CanAccessApi = true }));
    ...
    {
        await using var fixture = await AccountAdminFixture.CreateAsync();
        await fixture.Accounts.Service.CreateAccountAsync(new AccountCreateOptions
        {
            Username = "admin", Password = fixture.Accounts.Password, CanAccessApi = true
        });
