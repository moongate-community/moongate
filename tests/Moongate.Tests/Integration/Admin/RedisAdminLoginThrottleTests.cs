...
public sealed class RedisAdminLoginThrottleTests
{
    ...
    public async Task TryAcquireAsync_SharedLimit_RejectsExcessAttempts(bool sameUsername)
    {
        ...
        {
            Assert.True(await fixture.Throttle.TryAcquireAsync(sameUsername ? $"192.0.2.{i}" : "192.0.2.1", sameUsername ? "alice" : $"user{i}"));
        }
        ...
        {
            await foreach (var key in fixture.Redis.Connection.GetServer(endpoint).KeysAsync(pattern: fixture.Prefix + "throttle:*"))
            {
