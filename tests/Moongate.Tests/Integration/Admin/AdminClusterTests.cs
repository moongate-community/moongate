...
public sealed class AdminClusterTests
{
    ...
    public async Task LoginAndGame_ShareSessionAndRevocationWithoutGameAccountServices(string revoke)
    {
        ...
        var account = (await fixture.Backend.Accounts.Service.CreateAccountAsync(
                           new()
                           {
        ...
        var login = await new AdminLogin.AdminLoginClient(fixture.LoginChannel).LoginAsync(
                        new()
                        {
                            Username = "admin", Password = fixture.Backend.Accounts.Password
                        }
        ...
        await accounts.CreateAccountAsync(
            new() { Username = "created", Password = fixture.Backend.Accounts.Password },
            headers
        );
        ...
        Assert.Equal(
            ...
            (await Assert.ThrowsAsync<RpcException>(
                 () =>
                     new AdminAccounts.AdminAccountsClient(fixture.GameChannel).CreateAccountAsync(new(), headers)
                                                                               .ResponseAsync
             )).StatusCode
        );
        Assert.Equal(
        ...
        {
            case "logout":
                await new AdminSession.AdminSessionClient(fixture.GameChannel).LogoutAsync(new(), headers);

                break;
            case "revoke":
                await fixture.Backend.PeerAuthority.RevokeSessionsAsync(account.Id);

                break;
            case "role":
                await fixture.Backend.PeerAuthority.UpdateAccessAsync(
                    account.Id,
                    new() { CanAccessApi = true, AccountType = DomainAccountType.Regular }
                );

                break;
            case "disable":
                await fixture.Backend.PeerAuthority.SetApiAccessAsync("admin", false);
            ...
            default:
                ...
                await fixture.Backend
                             ...
                             .Connection
                             .GetDatabase()
                             .KeyDeleteAsync(fixture.Backend.Redis.Prefix + "session:" + digest);
        ...
        foreach (var channel in new[] { fixture.LoginChannel, fixture.GameChannel })
        {
            Assert.Equal(
                ...
                (await Assert.ThrowsAsync<RpcException>(
                     () =>
                         new AdminServer.AdminServerClient(channel).GetServerInfoAsync(new(), headers).ResponseAsync
                 )).StatusCode
            );
        }
    }
