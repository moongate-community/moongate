...
public sealed class WorldSaveBootstrapTests
{
        ...
        bool failCleanup, bool aggregateLoopFailure)
    {
        ...
        if (failCleanup)
        {
            container.RegisterMoongateService(new CallbackStartupService(() => Task.CompletedTask, () => throw cleanupFailure));
        }
        var bootstrap = new MoongateServerBootstrap(container, CancellationToken.None);
        await bootstrap.StartAsync();
    ...
    public void Registration_ResolvesOneSharedWorldSaveService()
    {
        ...
        container.RegisterInstance<IGameLoopService>(new Moongate.Server.Services.GameLoop.GameLoopService(
            new Moongate.Server.Core.Data.GameLoop.GameLoopOptions(), timers, TimeProvider.System));
