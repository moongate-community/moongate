...
public sealed class WorldSaveBootstrapTests
{
    ...
    public void Registration_ResolvesOneSharedWorldSaveService()
    {
        ...
        var persistence = new Moongate.Persistence.Services.MoongatePersistenceService(
            new Moongate.Persistence.Data.Config.PostgreSqlPersistenceOptions()
        );
        ...
        var timers = new Moongate.Server.Services.Timing.TimerWheelService(
            new Moongate.Server.Core.Data.Timing.TimerWheelOptions(),
            TimeProvider.System
        );
        container.RegisterInstance<ITimerService>(timers);
        container.RegisterInstance<IGameLoopService>(
            new Moongate.Server.Services.GameLoop.GameLoopService(
                new Moongate.Server.Core.Data.GameLoop.GameLoopOptions(),
                timers,
                TimeProvider.System
            )
        );
