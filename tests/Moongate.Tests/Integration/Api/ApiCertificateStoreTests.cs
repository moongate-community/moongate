...
public sealed class ApiCertificateStoreTests
{
    ...
    public void Load_MissingCertificate_GeneratesPasswordlessIdentityWithConfiguredNamesAndPublicCopy()
    {
        ...
        Assert.InRange(certificate.NotAfter.ToUniversalTime() - DateTime.UtcNow, TimeSpan.FromDays(364), TimeSpan.FromDays(366));
        Assert.False(Assert.Single(certificate.Extensions.OfType<X509BasicConstraintsExtension>()).CertificateAuthority);
        var usages = Assert.Single(certificate.Extensions.OfType<X509EnhancedKeyUsageExtension>()).EnhancedKeyUsages.Cast<Oid>().Select(oid => oid.Value);
    ...
    public async Task Load_ConcurrentCreators_PublishOneCompleteIdentity()
    {
        ...
        using var publicCertificate = X509CertificateLoader.LoadCertificateFromFile(Path.Combine(directories["config"], config.CertificatePath) + ".pem");
    ...
    public void Load_SeparateDirectories_GeneratesDistinctIdentities()
    {
        ...
        using var first = new ApiCertificateStore().Load(GenerationConfig(), new DirectoriesConfig(firstDirectory.Path, ["config"]), TimeProvider.System);
        using var second = new ApiCertificateStore().Load(GenerationConfig(), new DirectoriesConfig(secondDirectory.Path, ["config"]), TimeProvider.System);
        Assert.NotEqual(first.GetCertHashString(HashAlgorithmName.SHA256), second.GetCertHashString(HashAlgorithmName.SHA256));
    }

    [Theory, InlineData("corrupt"), InlineData("expired"), InlineData("future"), InlineData("client_only"), InlineData("no_private_key"), InlineData("wrong_password")]
    public void Load_ExistingInvalidCertificate_FailsWithoutReplacingIt(string invalidity)
    {
        ...
        if (invalidity == "corrupt") { File.WriteAllText(path, "invalid test certificate"); }
        else if (invalidity == "wrong_password") { Environment.SetEnvironmentVariable(fixture.Config.CertificatePasswordEnvironmentVariable, "wrong-test-password"); }
        else { fixture.UseInvalidServerCertificate(invalidity); }
        var bytes = File.ReadAllBytes(path);
        Assert.ThrowsAny<Exception>(() => new ApiCertificateStore().Load(fixture.Config, fixture.Directories, TimeProvider.System));
        Assert.Equal(bytes, File.ReadAllBytes(path));
    ...
    public void Load_ConfiguredPassword_EncryptsGeneratedPfx()
    {
        ...
        using var persisted = X509CertificateLoader.LoadPkcs12FromFile(path, fixture.Password, X509KeyStorageFlags.EphemeralKeySet);
        Assert.Equal(certificate.RawData, persisted.RawData);
    }
    ...
    public void Load_MissingConfiguredPassword_FailsBeforeCreatingFiles()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => new ApiCertificateStore().Load(config, directories, TimeProvider.System));
        Assert.False(Directory.Exists(Path.Combine(directories["config"], "tls")));
    }
