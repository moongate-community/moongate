...
public class Point3DListTests
{
    ...
    public void Add_BeyondInitialCapacity_PreservesOrderAndLastPoint()
    {
        ...
        list.Add(new Point3D(100, 200, -5));
        Assert.Equal(41, list.Count);
        Assert.Equal(new Point3D(0, 0, 5), list[0]);
        Assert.Equal(new Point3D(39, -39, 5), list[39]);
        Assert.Equal(new Point3D(100, 200, -5), list.Last);
    }
    ...
    public void ToArray_ReturnsIndependentPointsAndDrainsList()
    {
        ...
        list.Add(7, 8, 9);
        Assert.Equal(new Point3D(1, 2, 3), points[0]);
        Assert.Equal(new Point3D(7, 8, 9), list.Last);
    }
