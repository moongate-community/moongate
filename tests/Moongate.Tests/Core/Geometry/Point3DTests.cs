...
public class Point3DTests
{
    ...
    {
        Assert.True(new Point3D(1, 0, 0).CompareTo(new(2, 0, 0)) < 0);
        Assert.True(new Point3D(1, 1, 0).CompareTo(new(1, 0, 9)) > 0);
        Assert.True(new Point3D(1, 1, 1).CompareTo(new(1, 1, 2)) < 0);
    }
    ...
    {
        var p = new Point3D(new(5, 6), -3);

        Assert.Equal(new(5, 6, -3), p);
    }
    ...
    public void InRange_UsesChebyshevDistance_IgnoringZ()
    {
        ...
        Assert.True(origin.InRange(new(103, 98, 50), 3));
        Assert.False(origin.InRange(new(104, 98, 0), 3));
    }
    ...
    public void ToPoint2D_And_ImplicitConversion_DropZ()
    {
        ...
        Assert.Equal(new(7, 8), p3.ToPoint2D());
        Assert.Equal(new(7, 8), viaImplicit);
    }
    ...
    [Theory, InlineData("(1, 2)"), InlineData("(1, 2, 3, 4)"), InlineData("1, 2, 3")]
    public void TryParse_InvalidInput_ReturnsFalse(string input)
        => Assert.False(Point3D.TryParse(input, null, out _));

    [Theory, InlineData("(1, 2, 3)", 1, 2, 3), InlineData(" ( 1 ,2, 3 ) ", 1, 2, 3)]
