...
public class LootTemplatesLoaderTests
{
    ...
    public async Task LoadAsync_WhenMissing_SeedsCompleteDataset()
    {
        ...
        try
        {
            ...
            Assert.Equal(279, lootTemplates.All.Select(template => template.Id).Distinct(StringComparer.OrdinalIgnoreCase).Count());
    ...
    public async Task LoadAsync_ExistingDirectory_LoadsYamlRecursively()
    {
        var root = NewRoot();
        var directories = new DirectoriesConfig(root, Array.Empty<string>());
        WriteLoot(root, "weapons.yaml", Loot(id: "weapon_pack"));
        WriteLoot(root, "reagents/additive.yaml", Loot(id: "reagent_pack", mode: "Additive", entries: Entry("ItemTag: reagent", "Chance: 0.5", "AmountMin: 2", "AmountMax: 5")));
    ...
    public async Task LoadAsync_InvalidSecondFile_RegistersNothing()
    {
        var root = NewRoot();
        var directories = new DirectoriesConfig(root, Array.Empty<string>());
        WriteLoot(root, "a-valid.yaml", Loot(id: "valid_pack"));
        WriteLoot(root, "b-invalid.yaml", Loot(id: "invalid_pack", entries: Entry("ItemTemplateId: missing_item", "Weight: 1", "Amount: 1")));
        var service = new LootTemplateService();
        var loader = new LootTemplatesLoader(service, CreateItems(), directories);
    ...
    public static TheoryData<string, string, string, int?> InvalidDocuments()
    {
        return new TheoryData<string, string, string, int?>
        {
            ...
            { Loot(entries: Entry("ItemTemplateId: missing_item", "Weight: 1", "Amount: 1")), "Unknown ItemTemplateId", "loot", 0 },
            ...
            { Loot(entries: Entry("ItemTemplateId: broadsword", "Weight: 1", "Amount: 1", "AmountMin: 1", "AmountMax: 2")), "cannot combine Amount with a range", "loot", 0 },
            { Loot(entries: Entry("ItemTemplateId: broadsword", "Weight: 1", "AmountMin: 1")), "requires both AmountMin and AmountMax", "loot", 0 },
            { Loot(entries: Entry("ItemTemplateId: broadsword", "Weight: 1", "AmountMin: 2", "AmountMax: 1")), "AmountMin cannot exceed AmountMax", "loot", 0 },
            { Loot(entries: Entry("ItemTemplateId: broadsword", "Weight: 1", "Amount: 0")), "Amount must be greater than zero", "loot", 0 },
            { Loot(entries: Entry("ItemTemplateId: broadsword", "Weight: 1", "Amount: -1")), "Amount must be greater than zero", "loot", 0 },
            { Loot(entries: Entry("ItemTemplateId: broadsword", "Weight: 1", "AmountMin: 0", "AmountMax: 1")), "Amount range bounds must be greater than zero", "loot", 0 },
            { Loot(entries: Entry("ItemTemplateId: broadsword", "Weight: 1", "AmountMin: 1", "AmountMax: 0")), "Amount range bounds must be greater than zero", "loot", 0 },
            { Loot(entries: Entry("ItemTemplateId: broadsword", "Weight: 0", "Amount: 1")), "Weight must be greater than zero", "loot", 0 },
            { Loot(entries: Entry("ItemTemplateId: broadsword", "Weight: 1", "Chance: 0.5", "Amount: 1")), "Weighted entry cannot define Chance", "loot", 0 },
            { Loot(mode: "Additive", entries: Entry("ItemTemplateId: broadsword", "Amount: 1")), "Additive entry requires Chance", "loot", 0 },
            { Loot(mode: "Additive", entries: Entry("ItemTemplateId: broadsword", "Chance: 1.1", "Amount: 1")), "Chance must be between 0 and 1", "loot", 0 },
            { Loot(mode: "Additive", entries: Entry("ItemTemplateId: broadsword", "Chance: .nan", "Amount: 1")), "Chance must be finite", "loot", 0 },
            { Loot(mode: "Additive", entries: Entry("ItemTemplateId: broadsword", "Chance: .inf", "Amount: 1")), "Chance must be finite", "loot", 0 },
            { Loot(mode: "Additive", entries: Entry("ItemTemplateId: broadsword", "Chance: -.inf", "Amount: 1")), "Chance must be finite", "loot", 0 },
            { Loot(mode: "Additive", entries: Entry("ItemTemplateId: broadsword", "Weight: 1", "Chance: 0.5", "Amount: 1")), "Additive entry cannot define Weight", "loot", 0 }
        };
    }
    ...
    [Theory]
    [MemberData(nameof(ExplicitNullNonNullableYamlDocuments))]
    public async Task LoadAsync_ExplicitNullForNonNullableScalar_WrapsDeserializationFailureWithRelativePath(
        string yaml
    )
    {
        var root = NewRoot();
    ...
    private static ItemTemplateService CreateItems()
    {
        ...
        items.Register(new ItemTemplate { Id = "broadsword", Tags = ["weapon"] });
        items.Register(new ItemTemplate { Id = "black_pearl", Tags = ["reagent"] });
        return items;
    }
