...
public class WorldServiceTests
{
    ...
    {
        var time = new FixedTimeProvider(new(2026, 1, 1, 13, 37, 45, TimeSpan.Zero));

        var gameTime = Serialize(Service(new([]), time).BuildSequence(Player())[14]); // 0x5B is last
    ...
    public void BuildSequence_ItemAlreadyOnTheHairLayer_HairIsNotDrawn()
    {
        ...
        {
            Id = new(0x40000007), ItemId = 0x203C, EquippedLayer = LayerType.Hair
        };

        var incoming = Serialize(Service(new([wig])).BuildSequence(Player())[8]);
    ...
    public void BuildSequence_MobileIncoming_IncludesEquipmentAndHair()
    {
        ...
        var incoming = Serialize(Service(new([shirt])).BuildSequence(Player())[8]); // 0x78 is the 9th packet
    ...
    [Theory, InlineData(0, false, NotorietyType.Innocent), InlineData(0, true, NotorietyType.Criminal),
    ...
    public void BuildSequence_NotorietyDerivesFromKillsAndCriminal(int kills, bool criminal, NotorietyType expected)
    {
        ...
        var incoming = Serialize(Service(new([])).BuildSequence(mobile)[8]); // 0x78 is the 9th packet
    ...
    public void BuildSequence_SeasonComesFromTheMap()
    {
        ...
        var season = Serialize(Service(new([])).BuildSequence(Player())[3]); // 0xBC is the 4th packet
    ...
    {
        var mobile = Player();
        mobile.Skills[40] = new() { Value = 733, Cap = 1200, Lock = SkillLockType.Locked };

        var skills = Serialize(Service(new([])).BuildSequence(mobile)[11]); // 0x3A follows the locks

        Assert.Equal(0x3A, skills[0]);
        Assert.Equal(
            6 + 9 * 3,
            BinaryPrimitives.ReadUInt16BigEndian(skills.AsSpan(1))
        );                             // all three, not just the trained one
    ...
    public void BuildSequence_StatLockInfoCarriesTheMobileLocks()
    {
        ...
        var locks = Serialize(Service(new([])).BuildSequence(mobile)[10]); // 0xBF/0x19 follows 0x11
    ...
    public void BuildSequence_StatusCarriesStatCapAndFollowersFromTheMobile()
    {
        ...
        var status = Serialize(Service(new([])).BuildSequence(mobile)[9]); // 0x11 is the 10th packet
    ...
    {
        var status = Serialize(Service(new([])).BuildSequence(Player())[9]);
    ...
    public void BuildSequence_TwoItemsOnOneLayer_OnlyTheFirstIsDrawn()
    {
        ...
        var shirt = new ItemEntity
        {
            Id = new(0x40000005), ItemId = 0x1517, EquippedLayer = LayerType.Shirt
        };
        var otherShirt = new ItemEntity
        {
            Id = new(0x40000006), ItemId = 0x1518, EquippedLayer = LayerType.Shirt
        };

        var incoming = Serialize(Service(new([shirt, otherShirt])).BuildSequence(Player())[8]);
    ...
    public void BuildSequence_WarModeComesFromTheMobile()
    {
        ...
        var warMode = Serialize(Service(new([])).BuildSequence(mobile)[12]); // 0x72 is the 13th packet
    ...
    public void IsRecipient_ExcludedSerial_IsFalse()
    {
        ...
        Assert.False(
            WorldService.IsRecipient(SessionStateType.InWorld, character, 0, new(100, 100, 0), 18, new Serial(0x1))
        );
    }
    ...
    public void SendToPlayersInRange_NoSessions_ReturnsZero()
    {
        ...
        Assert.Equal(0, service.SendToPlayersInRange(0, new(100, 100, 0), 18, new NoopPacket()));
    }
