...
public class MapTileServiceTests
{
    ...
    public void TryGetWalkableZ_ClosestCandidateBlocked_FallsBackToFartherCandidate()
    {
        ...
        try
        {
            ...
            var found = service.TryGetWalkableZ(0, 1, 1, 20, [], out var newZ);
    ...
    {
        var dir = ClientDir(FlatLandId, 0);

        try
        {
            var service = Build(dir);
            var groundItem = new ItemEntity
                { Id = new(0x40000001), ItemId = ImpassableItemId, MapId = 0, Position = new(1, 1, 0) };

            var found = service.TryGetWalkableZ(0, 1, 1, 0, [groundItem], out _);
    ...
    public void TryGetWalkableZ_FlatTerrain_ReturnsLandZ()
    {
        ...
        try
        {
            ...
            var found = service.TryGetWalkableZ(0, 1, 1, 0, [], out var newZ);
    ...
    public void TryGetWalkableZ_GroundItemAtDifferentPosition_IsIgnored()
    {
        ...
        try
        {
            var service = Build(dir);
            var groundItem = new ItemEntity
                { Id = new(0x40000002), ItemId = ImpassableItemId, MapId = 0, Position = new(2, 2, 0) };

            var found = service.TryGetWalkableZ(0, 1, 1, 0, [groundItem], out var newZ);
    ...
    public void TryGetWalkableZ_InsufficientHeadroom_ReturnsFalse()
    {
        ...
        var dir = ClientDir(FlatLandId, 0, (LowCeilingStaticId, 1, 1, 4, 0));

        try
        {
            var service = Build(dir);

            var found = service.TryGetWalkableZ(0, 1, 1, 0, [], out _);
    ...
    public void TryGetWalkableZ_PlatformAtCurrentHeight_StaysOnPlatform()
    {
        ...
        var dir = ClientDir(FlatLandId, 0, (PlatformStaticId, 1, 1, 0, 0));

        try
        {
            var service = Build(dir);

            var found = service.TryGetWalkableZ(0, 1, 1, 4, [], out var newZ);
    ...
    public void TryGetWalkableZ_UnknownMap_ReturnsFalse()
    {
        ...
        try
        {
            ...
            var found = service.TryGetWalkableZ(99, 1, 1, 0, [], out _);
    ...
    {
        var dir = ClientDir(FlatLandId, 0, (WallStaticId, 1, 1, 0, 0));
    ...
    )
    {
        ...
        UltimaFixtures.SetItem(tileData, PlatformStaticId, (uint)TileFlagType.Surface, 4, "platform");

        // LowCeilingStaticId: impassable, but its base sits above the ground, leaving too little headroom.
        UltimaFixtures.SetItem(tileData, LowCeilingStaticId, (uint)TileFlagType.Impassable, 4, "low ceiling");

        // ImpassableItemId: the dynamic (ground-item) counterpart to WallStaticId.
        UltimaFixtures.SetItem(tileData, ImpassableItemId, (uint)TileFlagType.Impassable, 20, "crate");
        ...
        UltimaFixtures.SetItem(tileData, NearSurfaceId, (uint)TileFlagType.Surface, 0, "near surface");
