...
public sealed class BulletinBoardServiceTests
{
    ...
    private sealed class BulletinBoardInMemoryMessageRepository : IBulletinBoardMessageRepository
    {
        ...
        public ValueTask<IReadOnlyList<BulletinBoardMessageEntity>> GetByBoardIdAsync(
            ...
            => ValueTask.FromResult<IReadOnlyList<BulletinBoardMessageEntity>>(
                [.. _messages.Values.Where(m => m.BoardId == boardId).OrderBy(m => m.PostedAtUtc).Select(Clone)]
            );
    ...
    public async Task OpenBoardAsync_ShouldEnqueueDisplayAndSummaries()
    {
        var service = CreateService(
            out var itemService,
            out var queue,
            out var sessionService,
            out var persistenceService,
            out _
        );
    ...
    public async Task PostMessageAsync_ShouldPersistAndEnqueueSummary()
    {
        var service = CreateService(
            ...
            out var sessionService,
            out var persistenceService,
            out var characterService
        );
        var session = AddSession(sessionService, (Serial)0x00000055u);
        characterService.Character = new() { Id = session.CharacterId, Name = "Poster" };
        var board = CreateBoard();
        itemService.Items[board.Id] = board;
    ...
    private BulletinBoardMessagesPacket BuildPostPacket(
        uint boardId,
        uint parentId,
        string subject,
    ...
    private static BulletinBoardMessageEntity CreateMessage(
        Serial messageId,
        Serial boardId,
        Serial parentId,
        Serial ownerId,
    ...
    )
    {
        ...
        return new(itemService, characterService, sessionService, queue, persistenceService);
    }

    private static void WriteAscii(ref SpanWriter writer, string value)
    {
