...
public sealed class DyeColorServiceTests
{
    ...
    {
        public Action<PendingCursorCallback>? PendingCallback { get; private set; }
        public long LastSessionId { get; private set; }

        public Task HandleAsync(TargetRequestCursorEvent gameEvent, CancellationToken cancellationToken = default)
            => Task.CompletedTask;
    ...
    private sealed class DyeColorServiceTestSpatialWorldService : ISpatialWorldService
    {
        ...
        public IGameNetworkPacket? LastBroadcastPacket { get; private set; }
        ...
        public Task HandleAsync(DropItemToGroundEvent gameEvent, CancellationToken cancellationToken = default)
            => Task.CompletedTask;
    ...
    public async Task HandleResponseAsync_WhenPendingRequestExists_ShouldApplyHueAndBroadcastUpdate()
    {
        ...
        var handled = await service.HandleResponseAsync(
                          session,
                          new()
                          {
    ...
    )
    {
        ...
        return new(targetService, itemService, sessionService, queue, spatialWorldService, characterService);
    }
}
