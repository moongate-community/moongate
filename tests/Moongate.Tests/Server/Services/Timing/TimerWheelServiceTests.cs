...
public sealed class TimerWheelServiceTests
{
    ...
    public void CallbackFault_RemainsOriginalWhenDiagnosticClockAlsoFails()
    {
        ...
        var timers = new TimerWheelService(new(), faultingClock);
    ...
    {
        var clock = new ManualTimeProvider();
        var unbound = new TimerWheelService(new(), clock);
    ...
    {
        var clock = new ManualTimeProvider(1000);
        var timers = Create(clock, new() { TickDuration = TimeSpan.FromMilliseconds(1) });
    ...
    public void Registration_AtFractionalTimeSpanTickNeverRoundsEarly()
    {
        ...
        var timers = Create(clock, new() { TickDuration = TimeSpan.FromTicks(1), WheelSize = 2 });
    ...
    public void Repeat_FractionalCompletionOnlyCoalescesElapsedOccurrences(
        long firstTimestamp,
        long expectedCoalesced,
        long nextTimestamp
    )
    {
        var clock = new RawTimestampTimeProvider(30_000_000);
        var timers = new TimerWheelService(new() { TickDuration = TimeSpan.FromTicks(1) }, clock);
        timers.BindToCurrentThread(() => { });
    ...
    {
        var clock = new ManualTimeProvider();
        var timers = Create(clock, new() { MaxPendingTimers = 1 });
        timers.RegisterTimer(
            "repeat",
            TimeSpan.FromMilliseconds(8),
            () =>
