...
public class ItemBookServiceTests
{
    ...
    private sealed class TestMobileService : IMobileService
    {
        ...
        public Task CreateOrUpdateAsync(UOMobileEntity mobile, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public Task<bool> DeleteAsync(Serial id, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        ...
        public Task<List<UOMobileEntity>> GetPersistentMobilesInSectorAsync(int mapId, int sectorX, int sectorY, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public Task<UOMobileEntity> SpawnFromTemplateAsync(string templateId, Point3D location, int mapId, Serial? accountId = null, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public Task<(bool Spawned, UOMobileEntity? Mobile)> TrySpawnFromTemplateAsync(string templateId, Point3D location, int mapId, Serial? accountId = null, CancellationToken cancellationToken = default) => throw new NotSupportedException();
    }
    ...
    public async Task HandleBookHeaderAsync_WhenWritableBookInBackpack_ShouldPersistTitleAndAuthorAndEnqueueTooltip()
    {
        ...
        var queue = new BasePacketListenerTestOutgoingPacketQueue();
        var service = new ItemBookService(itemService, mobileService, queue);
        itemService.ItemsById[backpackId] = new UOItemEntity { Id = backpackId, ItemId = 0x0E75 };
        var item = new UOItemEntity
        {
        ...
        var session = new GameSession(new(client))
        {
            ...
            Character = new UOMobileEntity { Id = playerId }
        };
