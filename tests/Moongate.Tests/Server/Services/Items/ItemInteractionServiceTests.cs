...
public class ItemInteractionServiceTests
{
    private sealed class TestItemService : IItemService
    {
        ...
        public Task<bool> EquipItemAsync(Serial itemId, Serial mobileId, ItemLayerType layer) => throw new NotSupportedException();
        ...
        public Task<UOItemEntity?> GetItemAsync(Serial itemId) => Task.FromResult(ItemsById.TryGetValue(itemId, out var item) ? item : null);
    ...
    private sealed class TestBookService : IItemBookService
    {
        ...
        public Task<bool> TryEnqueueBookAsync(GameSession session, UOItemEntity item, CancellationToken cancellationToken = default)
        {
            _ = session;
            _ = cancellationToken;
        ...
        public Task<bool> HandleBookPagesAsync(GameSession session, Moongate.Network.Packets.Incoming.Books.BookPagesPacket packet, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public Task<bool> HandleBookHeaderAsync(GameSession session, Moongate.Network.Packets.Incoming.Books.BookHeaderNewPacket packet, CancellationToken cancellationToken = default) => throw new NotSupportedException();
    }

    private sealed class TestCharacterService : ICharacterService
    {
        public UOMobileEntity? Character { get; set; }
        public Task<bool> AddCharacterToAccountAsync(Serial accountId, Serial characterId) => throw new NotSupportedException();
        public Task ApplyStarterEquipmentHuesAsync(Serial characterId, short shirtHue, short pantsHue) => throw new NotSupportedException();
        public Task<Serial> CreateCharacterAsync(UOMobileEntity character) => throw new NotSupportedException();
        public Task<UOItemEntity?> GetBackpackWithItemsAsync(UOMobileEntity character) => throw new NotSupportedException();
        public Task<UOItemEntity?> GetBankBoxWithItemsAsync(UOMobileEntity character) => throw new NotSupportedException();
        public Task<UOMobileEntity?> GetCharacterAsync(Serial characterId) => Task.FromResult(Character);
        public Task<List<UOMobileEntity>> GetCharactersForAccountAsync(Serial accountId) => throw new NotSupportedException();
        public Task<bool> RemoveCharacterFromAccountAsync(Serial accountId, Serial characterId) => throw new NotSupportedException();
    }

    private sealed class TestScriptDispatcher : IItemScriptDispatcher
    {
        public bool HasHookResult { get; set; } = true;
        public bool HasHook(UOItemEntity item, string hookName) => HasHookResult;
        public Task<bool> DispatchAsync(ItemScriptContext context, CancellationToken cancellationToken = default) => throw new NotSupportedException();
    }
    ...
    public async Task HandleSingleClickAsync_ShouldPublishItemSingleClickEvent()
    {
        ...
        itemService.ItemsById[targetSerial] = new UOItemEntity { Id = targetSerial, ParentContainerId = (Serial)0x40000001u };
        using var client = new MoongateTCPClient(new(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp));
        var session = new GameSession(new(client));

        var handled = await service.HandleSingleClickAsync(session, new SingleClickPacket { TargetSerial = targetSerial });
    ...
    public async Task HandleSingleClickAsync_WhenGroundItemOutOfRangeAndRegular_ShouldNotPublishEvent()
    {
        ...
        itemService.ItemsById[targetSerial] = new UOItemEntity
        {
            Id = targetSerial,
            MapId = 0,
        ...
        var session = new GameSession(new(client))
        {
            ...
            Character = new UOMobileEntity { Id = (Serial)0x2u, MapId = 0, Location = new(10, 10, 0) }
        };
    ...
    public async Task HandleDoubleClickAsync_ShouldPublishItemDoubleClickEvent()
    {
        ...
        var handled = await service.HandleDoubleClickAsync(session, new DoubleClickPacket { TargetSerial = targetSerial });
    ...
    {
        TileData.ItemTable[0x0E75] = new ItemData(string.Empty, UOTileFlag.Container, 0, 0, 0, 0, 0, 0);
        var eventBus = new NetworkServiceTestGameEventBusService();
        var itemService = new TestItemService();
        var bookService = new TestBookService();
        var queue = new BasePacketListenerTestOutgoingPacketQueue();
    ...
    public async Task HandleDoubleClickAsync_WhenMobile_ShouldPublishEventAndEnqueuePaperdollForHumanoid()
    {
        ...
        {
            Assert.That(handled, Is.True);
            Assert.That(eventBus.Events.OfType<MobileDoubleClickEvent>().Single().MobileSerial, Is.EqualTo(targetSerial));
            Assert.That(queue.TryDequeue(out var outbound), Is.True);
