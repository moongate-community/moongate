...
public sealed class DiagnosticServiceTests
{
    ...
    {
        var provider = new ControlledMetricProvider();
        using var fixture = new DiagnosticServiceFixture([provider], new DiagnosticOptions { Enabled = false });
        await fixture.Service.StartAsync();
    ...
    public static TheoryData<MetricSample[]> InvalidSamples => new()
    {
        new[] { Sample(), Sample() },
    ...
    public async Task CollectAsync_SnapshotUsesEndTimeAndMonotonicDuration()
    {
        ...
        Assert.Equal(new DateTimeOffset(2026, 9, 18, 12, 0, 2, TimeSpan.Zero), first.CollectedAt);
