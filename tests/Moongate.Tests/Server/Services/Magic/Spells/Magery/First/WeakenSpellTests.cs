...
[TestFixture]
public sealed class WeakenSpellTests
{
    [Test]
    public void ApplyEffect_WhenTargetIsAlive_ShouldApplyStrengthPenaltyAndMarker()
    {
        var spell = new WeakenSpell();
        ...
        Assert.Multiple(
            () =>
            {
                Assert.That(target.EffectiveStrength, Is.EqualTo(40));
                Assert.That(target.RuntimeModifiers, Is.Not.Null);
                Assert.That(target.RuntimeModifiers!.StrengthBonus, Is.EqualTo(-10));
                Assert.That(target.TryGetCustomInteger("magic.weaken", out var marker), Is.True);
    ...
    public void ApplyEffect_WhenTargetIsNull_ShouldLeaveCasterUnchanged()
    {
        ...
        Assert.Multiple(
            () =>
            {
                Assert.That(caster.EffectiveStrength, Is.EqualTo(50));
