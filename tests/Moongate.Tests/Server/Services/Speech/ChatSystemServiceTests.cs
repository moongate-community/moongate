...
public sealed class ChatSystemServiceTests
{
    ...
    public async Task HandleChatActionAsync_ShouldApplyVoiceRulesAndKickUsers()
    {
        ...
        await service.HandleChatActionAsync(
            alice,
            BuildChatPacket(ChatActionType.RestrictDefaultSpeakingPrivileges, string.Empty)
        );
        Drain(queue);
        ...
        Assert.That(
            blockedPackets.Any(
                p => p.SessionId == alice.SessionId && p.Packet.Command == 57 && p.Packet.Param2 == "blocked"
            ),
            Is.False
        );
        ...
        Assert.That(
            allowedPackets.Any(
                p => p.SessionId == alice.SessionId && p.Packet.Command == 57 && p.Packet.Param2 == "allowed"
            ),
            Is.True
        );
    ...
    public async Task HandleChatActionAsync_ShouldCreateAndJoinChannels()
    {
        ...
        var alicePackets = Drain(queue)
                           .Where(x => x.SessionId == alice.SessionId)
                           .Select(x => (ChatCommandPacket)x.Packet)
                           .ToList();

        Assert.That(
            alicePackets.Any(p => p.Command == (ushort)ChatCommandType.JoinedChannel && p.Param1 == "General"),
            Is.True
        );
        Assert.That(
            alicePackets.Any(
                p => p.Command == (ushort)ChatCommandType.AddUserToChannel &&
                     p.Param1.EndsWith("Alice", StringComparison.Ordinal)
            ),
            Is.True
        );
        ...
        Assert.That(
            packets.Any(
                p => p.SessionId == bob.SessionId &&
                     p.Packet.Command == (ushort)ChatCommandType.JoinedChannel &&
                     p.Packet.Param1 == "General"
            ),
            Is.True
        );
        Assert.That(
            packets.Any(
                p => p.Packet.Command == (ushort)ChatCommandType.AddUserToChannel &&
                     p.Packet.Param1.EndsWith("Bob", StringComparison.Ordinal)
            ),
            Is.True
        );
    }
    ...
    public async Task HandleChatActionAsync_ShouldRenameProtectAndWhoIsChannels()
    {
        ...
        Assert.That(
            renamePackets.Any(
                p => p.Packet.Command == (ushort)ChatCommandType.JoinedChannel && p.Packet.Param1 == "Renamed"
            ),
            Is.True
        );
        ...
        Assert.That(
            visibleWhoIs.Any(p => p.SessionId == alice.SessionId && p.Packet.Command == 43 && p.Packet.Param2 == "Bob"),
            Is.True
        );
        ...
        Assert.That(
            emotePackets.Any(p => p.SessionId == alice.SessionId && p.Packet.Command == 58 && p.Packet.Param2 == "waves"),
            Is.True
        );
    }
    ...
    public async Task HandleChatActionAsync_ShouldRespectIgnoreAndPrivateMessageToggles()
    {
        ...
        Assert.That(
            firstAttempt.Any(p => p.SessionId == bob.SessionId && p.Packet.Command == 59 && p.Packet.Param2 == "hello"),
            Is.True
        );
        ...
        Assert.That(
            secondAttempt.Any(p => p.SessionId == bob.SessionId && p.Packet.Command == 59 && p.Packet.Param2 == "blocked"),
            Is.False
        );
    }
    ...
    public async Task HandleChatActionAsync_ShouldRouteChannelMessagesToMembers()
    {
        ...
        Assert.That(
            packets.Any(
                p => p.SessionId == bob.SessionId && p.Packet.Command == 57 && p.Packet.Param2 == "hello conference"
            ),
            Is.True
        );
    }
    ...
    private ChatSystemService CreateService(
        out BasePacketListenerTestOutgoingPacketQueue queue,
        out ChatSystemTestGameNetworkSessionService sessionService
    )
    {
        queue = new();
        sessionService = new();

        return new(queue, sessionService);
    }
    ...
    private static List<OutgoingGamePacket> Drain(BasePacketListenerTestOutgoingPacketQueue queue)
    {
