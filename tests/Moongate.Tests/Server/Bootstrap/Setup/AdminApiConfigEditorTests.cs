...
public sealed class AdminApiConfigEditorTests
{
    ...
    public void EnableGeneratedCertificate_ExistingTable_PreservesCommentsAndOtherTables(string newline)
    {
        var source = string.Join(newline,
            "# Configuration", "[admin_api] # Admin", "'enabled' = false # switch", "listen_address = '0.0.0.0'", "port = 2590", "[redis]", "connection_string = '${REDIS}'", "");
    ...
    {
        Assert.Throws<InvalidOperationException>(() => AdminApiConfigEditor.EnableGeneratedCertificate(
