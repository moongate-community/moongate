...
public class MoongateHttpServicePortalEndpointTests
{
    ...
    public async Task PortalCharacterInventoryEndpoint_WhenAuthenticated_ShouldReturnEquippedAndBackpackItems()
    {
        ...
        try
        {
            ...
            var response = await http.GetAsync($"http://127.0.0.1:{port}/api/portal/characters/{character.Id.Value}/inventory");
            using var payloadDoc = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
            var root = payloadDoc.RootElement;
            ...
            Assert.Multiple(
                () =>
                {
                    ...
                    Assert.That(root.GetProperty("items")[0].GetProperty("location").GetString(), Is.EqualTo("Equipped: Backpack"));
                    Assert.That(root.GetProperty("items")[1].GetProperty("location").GetString(), Is.EqualTo("Equipped: OneHanded"));
                    ...
                    Assert.That(root.GetProperty("items")[4].GetProperty("location").GetString(), Is.EqualTo("Container: Pouch"));
                    Assert.That(root.GetProperty("items")[4].GetProperty("imageUrl").GetString(), Does.Contain("/api/item-templates/by-item-id/"));
                    ...
                    Assert.That(root.GetProperty("bankItems")[2].GetProperty("location").GetString(), Is.EqualTo("Container: Metal Chest"));
                }
            );
        }
        finally
    ...
    public async Task PortalCharacterInventoryEndpoint_WhenCharacterDoesNotBelongToAccount_ShouldReturnNotFound()
    {
        ...
        try
        {
            ...
            var response = await http.GetAsync($"http://127.0.0.1:{port}/api/portal/characters/{foreignCharacter.Id.Value}/inventory");
