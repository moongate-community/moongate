...
public sealed class ApiConfigTests
{
    [Theory, InlineData("missing_path"), InlineData("null_password"), InlineData("null_dns"),
     InlineData("null_ips"), InlineData("empty_names"), InlineData("bad_dns"), InlineData("bad_ip"), InlineData("scoped_ip"),
     InlineData("zero_scope"), InlineData("wildcard")]
    public void Validate_DisabledGenerationWithInvalidCertificateSettings_Rejects(string invalidity)
    {
        ...
        switch (invalidity)
        {
            case "missing_path":
                ...
                break;
            case "null_password":
                config.CertificatePasswordEnvironmentVariable = null!;

                break;
            case "null_dns":
                config.CertificateDnsNames = null!;

                break;
            case "null_ips":
                config.CertificateIpAddresses = null!;

                break;
            case "empty_names":
                config.CertificateDnsNames = [];
                config.CertificateIpAddresses = [];

                break;
            case "bad_dns":
                config.CertificateDnsNames = ["https://realm.internal"];

                break;
            case "bad_ip":
                config.CertificateIpAddresses = ["realm.internal"];

                break;
            case "zero_scope":
                config.CertificateIpAddresses = ["fe80::1%0"];

                break;
            case "wildcard":
                config.CertificateDnsNames = ["*.internal"];

                break;
            case "scoped_ip":
                config.CertificateIpAddresses = ["fe80::1%3"];
    ...
    public void Validate_EnabledWithInvalidTlsPolicy_Rejects(string invalidField)
    {
        ...
        switch (invalidField)
        {
            ...
            case "peers":
                ...
                break;
            case "fingerprint":
                config.Peers[0].CertificateSha256 = new('Z', 64);
            ...
            case "identity":
                ...
                break;
            case "zero_operation":
                config.Peers[0].AllowedOperations = new([0]);

                break;
            case "duplicate":
                config.Peers =
                [
                    config.Peers[0], new() { CertificateSha256 = new('a', 64), PeerId = "second" }
                ];

                break;
            case "password_variable":
                config.CertificatePasswordEnvironmentVariable = null!;
    ...
    private static ApiConfig ValidConfig()
        => new()
        {
            ...
            [
                new()
                {
                    CertificateSha256 = new('A', 64), PeerId = "peer", AllowedOperations = new([100])
                }
            ]
        };
}
