...
public sealed class PluginContainerExtensionsTests
{
    ...
    public void BatchOverload_OrdersDependenciesAndSupportsChaining()
    {
        ...
        var root = new RecordingPlugin(new("root", "Root", new(1, 0, 0)));
        var app = new RecordingPlugin(
            new(
                "app",
                "App",
                new(1, 0, 0),
                dependencies: [new("root")]
            )
        );
    ...
    {
        using var container = new Container();
        var instance = new RecordingPlugin(new("configured", "Configured", new(1, 0, 0)));
    ...
    public void Overloads_RejectNullContainerOrInput()
    {
        ...
        Assert.Throws<ArgumentNullException>(() => container.RegisterMoongatePlugin(null!));
        Assert.Throws<ArgumentNullException>(() => container.RegisterMoongatePlugins(null!));
    }
    ...
    public void Overloads_ShareDuplicateDetection()
    {
        using var container = new Container();
        container.RegisterMoongatePlugin<DefaultRegistrationPlugin>();
        var duplicate = new RecordingPlugin(new("DEFAULT", "Duplicate", new(2, 0, 0)));
