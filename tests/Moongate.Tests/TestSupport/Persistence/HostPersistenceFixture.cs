...
public sealed class HostPersistenceFixture : IAsyncDisposable
{
    ...
    private HostPersistenceFixture(PostgreSqlTestDatabase database, PostgreSqlTestDatabase? accountsDatabase, bool autoSync)
    {
        ...
        if (accountsDatabase is not null) { targets.Add(new(PersistenceDatabaseTarget.Accounts, accountsDatabase.ConnectionString)); }
    ...
    {
        return new HostPersistenceFixture(await new PostgreSqlFixture().CreateDatabaseAsync(),
            twoTargets ? await new PostgreSqlFixture().CreateDatabaseAsync() : null, autoSync);
    }
    ...
    {
        if (!Container.IsDisposed) { await Owner.DisposeAsync(); Container.Dispose(); }
        await Database.DisposeAsync();
        if (AccountsDatabase is not null) { await AccountsDatabase.DisposeAsync(); }
    }
}
