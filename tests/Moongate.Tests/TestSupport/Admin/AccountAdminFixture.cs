...
internal sealed class AccountAdminFixture : IAsyncDisposable
{
    ...
    private AccountAdminFixture(
        ...
        Container peer,
        TemporaryPersistenceDirectory directory
    )
    {
        Accounts = accounts;
        Redis = redis;
        ...
        Authority = new(accounts.Persistence, accounts.Accounts, Store, new(TimeSpan.FromMinutes(30)));
        PeerAuthority = new(
            peer.Resolve<MoongatePersistenceService>(),
            peer.Resolve<IDataAccess<AccountEntity>>(),
            redis.Store,
