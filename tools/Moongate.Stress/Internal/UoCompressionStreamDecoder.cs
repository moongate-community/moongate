...
internal sealed class UoCompressionStreamDecoder
{
    ...
    private readonly Dictionary<int, Node> _tree = BuildTree();
    ...
    public bool TryDecodeOne(IReadOnlyList<byte> input, out int consumedBytes, out byte[] payload)
    {
        consumedBytes = 0;
        ...
        for (var bitIndex = 0; bitIndex < totalBits; bitIndex++)
        {
            ...
            if (!_tree.TryGetValue(key, out var node))
            {
                return false;
            }
    ...
    private static Dictionary<int, Node> BuildTree()
    {
        var tree = new Dictionary<int, Node>();
        var nextPosition = 1;

        for (var i = 0; i < HuffmanTable.Length; i += 2)
        {
            var codeLength = HuffmanTable[i];
            var codeValue = HuffmanTable[i + 1];
            var byteValue = i / 2;
